...
import { GroupFee, ResponseValue } from '@app/shared/models';
...
@Component({
  selector: 'modal-group-fee',
  templateUrl: './modal-group-fee.component.html',
  styleUrls: ['./modal-group-fee.component.css']
})
export class ModalGroupFeeComponent implements OnInit {
  public entity: GroupFee;
  ...
  constructor(private _notificationService: NotificationService, private groupFeeService: GroupFeeService,
  ...
  edit(id: string, flag: boolean) {
    this.groupFeeService.getDetail(id).subscribe((res: ResponseValue<GroupFee>) => {
  ...
  saveChange(form: NgForm) {
    if (form.valid) {
      ...
      if (this.entity.id == undefined) {
        this.groupFeeService.add(this.entity).subscribe((res: number) => {
        ...
        this.groupFeeService.update(this.entity).subscribe((res: number) => {
          this.modalAddEdit.hide();
          form.resetForm();
