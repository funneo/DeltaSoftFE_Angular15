...
@Component({
  selector: 'modal-tollroute',
  templateUrl: './modal-tollroute.component.html',
  styleUrls: ['./modal-tollroute.component.css']
})
export class ModalTollrouteComponent implements OnInit {
  public entity:Tollroute;
  ...
  edit(id: number,flag:boolean) {
    ...
    this.tollRouteService.getDetail(id).subscribe((res: ResponseValue<Tollroute>) => {
      if (res.code == '200' || res.code == '201') {
  ...
  saveChange(form: NgForm) {
    if (form.valid) {
      ...
      if (this.entity.id == undefined) {
        this.tollRouteService.add(this.entity).subscribe((res: ResponseValue<any>) => {
          if (res.code == '200' || res.code == '201') {
        ...
        this.tollRouteService.update(this.entity).subscribe((res: ResponseValue<any>) => {
          if (res.code == '200' || res.code == '201') {
            this.modalAddEdit.hide();
            form.resetForm();
