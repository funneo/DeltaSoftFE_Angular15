...
import { AccountList, AccountingDetail, Accounts, Branch, Customer, Employee, OtherCategories, PaymentDetail, PrintForm, ResponseValue, Supplier } from '@app/shared/models';
...
import { AccountListService, AccountsService, AuthService, BranchService, CustomerService, EmployeeService, NotificationService, OtherCategoriesService, PaymentsService, UtilityService } from '@app/shared/services';
import { ModalDirective } from 'ngx-bootstrap/modal';
import { Subscription } from 'rxjs';
import { AccountsPaymentDetail } from '@app/shared/models/accounts-payment-detail.model';

@Component({
  selector: 'modal-phieu-chi-multi',
  templateUrl: './modal-phieu-chi-multi.component.html',
  styleUrls: ['./modal-phieu-chi-multi.component.css']
})
export class ModalPhieuChiMultiComponent implements OnInit {
  entity: Accounts;
  ...
  constructor(private _notificationService: NotificationService, private accountsService: AccountsService,
    ...
    private suppliertSerive: SupplierService, private paymentService: PaymentsService) {
  ...
  add(item: any = null) {
    ...
    if (item != undefined && item != null) {
      ...
      this.paymentService.getNonPaymented(item.id).subscribe((res: ResponseValue<PaymentDetail[]>) => {
        if (res.code == '200' || res.code == '201') {
          this.listPaymentDetails = res.data;
          this.listPaymentDetails.forEach(it => {
            if (it.id === item.id) {
              it.checked = true;
  ...
  edit(id: string, flag: boolean) {
    this.accountsService.getDetail(id).subscribe((res: ResponseValue<Accounts>) => {
      if (res.code == '200' || res.code == '201') {
        ...
        this.listPaymentDetails = this.entity.accountPayments;
        if (!this.flagXem)
          this.inputTen(1);
        this.modalAddEdit.show();
      }
  ...
  saveChange(form: NgForm) {
    if (form.valid) {
      ...
      if (this.entity.id == undefined) {
        this.accountsService.add2(this.entity).subscribe((res: ResponseValue<any>) => {
          if (res.code == '200' || res.code == '201') {
  ...
  isAllChecked() {
    if (this.listPaymentDetails)
      return this.listPaymentDetails.every(_ => _.checked);
  }

  checkAll(ev) {
    this.selectedItem = false;
    this.listPaymentDetails.forEach(x => x.checked = ev.target.checked)
    let total = 0;
    this.listPaymentDetails.forEach(it => {
      if (it.checked) total += it.amountAfterVAT;
      this.selectedItem = true;
    })
    this.entity.amount = total;
  }

  calculator(item: PaymentDetail) {
    this.selectedItem = false;
    item.checked = !item.checked;
    let total = 0;
    this.listPaymentDetails.forEach(it => {
      if (it.checked) {
        total += it.amountAfterVAT;
        this.selectedItem = true;
      }
    })
    this.entity.amount = total;
