...
@Component({
  selector: 'app-external-oil-purchased',
  templateUrl: './external-oil-purchased.component.html',
  styleUrls: ['./external-oil-purchased.component.css']
})
export class ExternalOilPurchasedComponent implements OnInit {
  pageIndex = 1;
  pageSize = 20;
  totalRows = 0;
  flagEdit = false;
  ...
  // viewModalPayment and viewModalReceipt are declared above
  ...
  loadData(): void {
    ...
    // .set('usergroupid')
    this.busy = this.externalOilPurchasedService.getPaging(params, false).subscribe((res: ResponseValue<Pagination<ExternalOilPurchased>>) => {
  ...
  @ViewChild('modalPhieuChi', { static: false }) modalPhieuChi: ModalPhieuChiComponent;
  @ViewChild('modalPhieuThu', { static: false }) modalPhieuThu: ModalPhieuThuComponent;

  checkShowPayment(item: ExternalOilPurchased): boolean {
    if (item.status < 1 || item.isCompleted) return false;
    // Check if refDate is > 18/11/2025
    // Note: refDate might be a string or Date object depending on API. 
    // Assuming string 'YYYY-MM-DD...' or Date object. 
    // moment(item.refDate) handles both safely.
    // However, let's look at existing code usage. 
    // Existing code uses: this.datepipe.transform(data.refDate, 'dd/MM/yyyy')

    // We want to allow if date > 2025-11-18.
    const cutoffDate = moment('2025-12-18T23:59:59.999');
    const refDate = moment(item.createdDate);
    return refDate.isAfter(cutoffDate);
