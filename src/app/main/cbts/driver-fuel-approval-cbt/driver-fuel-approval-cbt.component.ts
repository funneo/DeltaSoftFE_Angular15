...
@Component({
  selector: 'app-driver-fuel-approval-cbt',
  templateUrl: './driver-fuel-approval-cbt.component.html',
  styleUrls: ['./driver-fuel-approval-cbt.component.css']
})
export class DriverFuelApprovalCbtComponent implements OnInit {
  pageIndex = 1;
  pageSize = 9999;
  totalRows = 0;
  flagEdit = false;
  ...
  export(){
    ...
    .set('type','0')
     // .set('usergroupid')
     this.busy = this.driverFuelApprovalService.getPaging(params,true).subscribe((res: ResponseValue<Pagination<DriverFuelApproval>>) => {
  ...
  igas(){
    this.spinner.show();
    this.busy = this.driverFuelApprovalService.updateIgas().subscribe((res: ResponseValue<Pagination<DriverFuelApproval>>) => {
      if (res.code == '200' || res.code == '201') {
        this.loadData();
        this.filter();
        this.spinner.hide();
  ...
  edit(flag:boolean): void {
    ...
    this.viewModal = true;
    let permission=this.listDriverFuelApproval[index].createdBy==this.userLoged.id;
    setTimeout(() => {
      this.modalAddEdit.edit(this.listDriverFuelApproval[index].id, flag,permission );
    }, 50);
  }
