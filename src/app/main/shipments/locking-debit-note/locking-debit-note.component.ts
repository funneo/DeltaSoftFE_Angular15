...
@Component({
  selector: "app-locking-debit-note",
  templateUrl: "./locking-debit-note.component.html",
  styleUrls: ["./locking-debit-note.component.css"],
})
export class LockingDebitNoteComponent implements OnInit {
  pageIndex = 1;
  pageSize = 99999;
  ...
  ngOnInit(): void {
    var p=UtilityService.getLocalParams('LOCKINGDEBIT');
    localStorage.removeItem('LOCKINGDEBIT');
    if(p!=null){
 ...
 edit(id: number, flag: boolean): void {
    ...
    UtilityService.setLocalParams(p,"LOCKINGDEBIT");
    if (id == null) {
  ...
  lockingConfirm(): void {
    ...
    this.notificationService.printConfirmationDialog(
      MessageContstants.CONFIRM_LOCKING_MSG,
      () => this.locked(checks.join(","))
    );
  }

  locked(listIds: string): void {
    this.service.locked(listIds).subscribe((res: ResponseValue<any>) => {
      if (res.code == '200' || res.code == '201') {
        this.loadData();
      }
      else {
