...
import { ModalCsRatingComponent } from "@app/shared/components/shipments/modal-cs-rating/modal-cs-rating.component";
...
import { ModalShipmentViewSearchComponent } from "@app/shared/components/shipments/modal-shipment-view-search/modal-shipment-view-search.component";
import { SystemContstants } from "@app/shared/constants/SystemConstants";
import { ModalListDispatchOrderComponent } from "@app/shared/components/transports/modal-list-dispatch-order/modal-list-dispatch-order.component";
import { ModalDebtDebitnotesComponent } from "@app/shared/components/accounting/modal-debt-debitnotes/modal-debt-debitnotes.component";
...
@Component({
  ...
  styleUrls: ["./debit-note-detail.component.css"],
})
export class DebitNoteDetailComponent implements OnInit {
  ...
  _ngayHachtoan: string = moment(new Date()).format("DD/MM/YYYY");
  dateTimeOptions2 = this._utilityService.dateTimeOptionDays(new Date(), false);
  ...
  @ViewChild(ModalDebtDebitnotesComponent, { static: false })
  modalAddEdit: ModalDebtDebitnotesComponent;
  ...
  constructor(
    ...
    private paymentCbtService: PaymentCbtService,
    ...
    this._accept = this.authService.hasPermission("DEBITNOTES_ACCEPT");
  ...
  //    this.debitNotesService.locked(this.entity.id,true).subscribe((res: ResponseValue<any>) => {
  ...
  ngOnInit(): void {
    ...
    this._shipmentId = parseInt(
      this.activatedRoute.snapshot.params["shipmentId"]
    );
    this.loadChiNhanh();
    this.loadFee();
    this.loadCustomer();
    ...
    this.entity = {
      ...
      debitDate: moment(new Date()).format("DD/MM/YYYY"),
    };
    ...
    if (id != undefined && id != null) {
      ...
      this.loadEmployee();
      this.inputTen();
      if (this._type == 1) this.loadSupplier();
    }
  }
  ...
  showImport(): void {
    if (this._isChuyeduyet) return;
    this.viewImportExcel = true;
    setTimeout(() => {
      this.modalImport.view(0);
    }, 50);
  }
  ...
  loadShipment(): void {
    if (this._customerId == null) return;
    if (
  ...
  getThongTinJob(item: Shipment): void {
    if (item) {
      ...
      if (this.entity.id == undefined) {
        ...
        if (
          this.listDebitTypes?.findIndex(
            (x) => x.categoryCode == item.shipmentTypeName
          ) != -1
        ) {
  ...
  loadFee(): void {
    ...
    this.feeService.getAll(params).subscribe((res: ResponseValue<Fee[]>) => {
      // Filter and create a deep copy to avoid mutating cached objects
  ...
  onChangeFee(event: any, item: any): void {
    if (event && !event.status) {
      this._notificationService.printAlert(
        "Lỗi nhập liệu",
        "Mã phí đã bị khóa, không được sử dụng"
      );
      setTimeout(() => {
        item.feeId = null;
      });
  ...
  loadOtherCategory(): void {
    ...
    this.otherCategoriesService
      ...
      .subscribe((res: ResponseValue<OtherCategories[]>) => {
        this.listDebitTypes = [
          ...res.data.filter((x) => x.type === "DEBIT_TYPE"),
        ];
  ...
  selectedNgayDoanhThu(event) {
    let t = false;
    t = this._utilityService.validateDebitNoteDate(
      moment(event.start).format("DD/MM/YYYY")
    );
    if (!t) {
      this._notificationService.printAlert(
        "Lỗi nhập liêu",
        "Ngày doanh thu không hơp lệ!"
      );
      this._ngayDoanhThu = "";
      return;
    } else this._ngayDoanhThu = moment(event.start).format("DD/MM/YYYY");
    if (this.flagNew) this._ngayHachtoan = this._ngayDoanhThu;
  }
  selectedNgayHachtoan(event) {
    let t = false;
    t = this._utilityService.validateNgayHachtoanVsDoanhthu(
      moment(event.start).format("DD/MM/YYYY"), this._ngayDoanhThu
    );
    if (!t) {
      this._notificationService.printAlert(
        "Lỗi nhập liêu",
        "Tháng vận hành không hợp lệ!"
      );
      this._ngayHachtoan = "";
      return;
    } else this._ngayHachtoan = moment(event.start).format("DD/MM/YYYY");
  }

  isFeeLocked(fee: Fee): boolean {
    ...
    this.debitNotesService
      ...
      .subscribe((res: ResponseValue<DebitNotes>) => {
        if (res.code == "200" || res.code == "201") {
          ...
          this.flagLocked = (this.entity.step > 0 || this.entity.isLocked);
          ...
          this.flagNew = false;
          if (
            this.listPermissionCS.findIndex(
              (z) => z.customerId == this._customerId && z.isAcceptDebit
            ) == -1
          ) {
            this._accept = false;
          }
          ...
          if (this.entity.debitDate) {
            this.dateTimeOptions2 = this._utilityService.dateTimeOptionDays(
              new Date(
                moment(this.entity.debitDate, FormatContstants.DATEEN).format(
                  FormatContstants.DATEEN
                )
              ),
              false
            );
            this._ngayDoanhThu = moment(
              this.entity.debitDate,
              FormatContstants.DATEEN
            ).format(FormatContstants.DATEVN);
          }
          ...
          if (this.entity.accountingDate) {
            this.dateTimeOptions2 = this._utilityService.dateTimeOptionDays(
              new Date(
                moment(
                  this.entity.accountingDate,
                  FormatContstants.DATEEN
                ).format(FormatContstants.DATEEN)
              ),
              false
            );
            this._ngayHachtoan = moment(
              this.entity.accountingDate,
              FormatContstants.DATEEN
            ).format(FormatContstants.DATEVN);
          }
          ...
          //Kiểm tra nếu debit đã khóa thì ko cho thực hiện gì hết
          if (this.entity.step > 1) this.flagXem = true;
          if (!this.flagXem) this.inputTen();
          this.flagSave = false;
          this.loadShipment();
          this.loadEmployee();
  ...
  save(frm: NgForm): void {
    if (frm.valid && !this.flagSave) {
      ...
      if (this._ngayHachtoan) {
        if (moment(this._ngayHachtoan, FormatContstants.DATEVN).isValid()) {
          this.entity.accountingDate = moment(
            this._ngayHachtoan,
            FormatContstants.DATEVN
          ).format(FormatContstants.CLIENTDATE);
        } else {
      ...
      if (this.entity.debitNoteDetails.length < 1) {
        this._notificationService.printErrorMessage(
          "Chi tiết DebitNote không hợp lệ, kiểm tra lại!"
        );
        return;
      }
      this.entity.quantityOfGgoods = this._luongHang;
      if (this.entity.id == undefined) {
        this.debitNotesService.add(this.entity).subscribe(
        ...
        this.debitNotesService.update(this.entity).subscribe(
          (res: ResponseValue<any>) => {
            if (res.code == "200" || res.code == "201") {
              // frm.resetForm();
              this._notificationService.printSuccessMessage(
  ...
  onChangeTien(data: DebitNoteDetail) {
    ...
    let dg = +(data.price ?? 0);
    if (sl > 0 && dg > 0) data.amount = sl * dg;
    let s1 = +(data.amount ?? 0);
    let rvat = +(data.rVat ?? 0);
    data.vat =
      data.currency == "VND"
        ? Math.round((s1 * rvat) / 100)
        : Math.round(s1 * rvat) / 100;
    let vat = +(data.vat ?? 0);
    data.amountAfterVAT = s1 + vat;
    this.sumTien();
  ...
  submit(entity: DebitNoteDetail, key: string): void {
    ...
    this.debitNotesService.accept(item).subscribe(
      (res: ResponseValue<any>) => {
        if (res.code == "200" || res.code == "201") {
          this.localtion.back();
        } else {
  ...
  viewRating: boolean;
  @ViewChild(ModalCsRatingComponent, { static: false })
  modalRating: ModalCsRatingComponent;
  showRating(): void {
  ...
  getCP03(): void {
    if (
      this.listChiPhiLoHang?.findIndex((x) => x.feeCode.startsWith("CP03")) !=
      -1
    ) {
  ...
  getDef(): void {
    if (this.listDetail?.length > 1) {
      this._notificationService.printConfirmationDialog(
        "Bạn muốn lấy mã phí mặc định cho Debit-Note?",
        () => this.okDef()
      );
    } else {
      this.okDef();
  ...
  add(): void {
    ...
    this.entity = {
      ...
      type: item.type,
      refDate: moment(new Date()).format("DD/MM/YYYY"),
      debitDate: moment(item.debitDate).format("DD/MM/YYYY"),
      customerId: item.customerId,
      shipmentId: item.shipmentId,
      debitBranchId: item.debitBranchId,
      step: 0
    };
    this.flagLocked = false;
    this._isChuyeduyet = false;
    this._ngayLamDebit = moment(new Date()).format("DD/MM/YYYY");
    this.dateTimeOptions = this._utilityService.dateTimeOptionDays(
      new Date(),
      false
    );
    this._ngayDoanhThu = moment(item.debitDate).format("DD/MM/YYYY");
    this.dateTimeOptions2 = this._utilityService.dateTimeOptionDays(
      new Date(),
      false
    );
  ...
  saveSuccessImport(event: DebitNoteDetail[]): void {
  ...
  @ViewChild(ModalShipmentViewSearchComponent, { static: false })
  modalSearchJob: ModalShipmentViewSearchComponent;
  FindJob(): void {
  ...
  showPrint(kieu: number) {
    setTimeout(() => {
      this.debitNotesService
        .getDetail(this.entity.id.toString())
        .subscribe((res: ResponseValue<DebitNotes>) => {
          if (res.code == "200" || res.code == "201") {
  ...
  print(entity: DebitNotes, kieu: number) {
    ...
    if (entity.tenKhachHang?.toLowerCase().includes("dsv"))
      entity.debitNo = entity.debitNo.substring(2);
    ...
    if (list != null) {
      ...
      if (index != -1) noiDung = listMauIn[index].content;
    }
    // console.log(noiDung);
