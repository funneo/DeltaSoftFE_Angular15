...
3. ✅ Đợt 1 BE (2026-09-13, build 0 lỗi, CHƯA commit): model/repo (`FuelDgas3Outbound`/`Completion`) + adapter HttpClient `IDgas3ApiClient`/`Dgas3ApiClient` gọi §2 + `FuelDgas3MonitorController` (JWT, gate F047, `GetPaging`+`Resend`). `appsettings.json` thêm `Dgas3BaseUrl`/`Dgas3ApiKey` (outbound, điền sau khi có từ đối tác). ⬜ Chờ anh review code + chạy `Migration_FuelDgas3_GetById_20260913.sql` (SP additive `SP_FuelDgas3Outbound_GetById` cho nút "Gửi lại" với dòng Failed-NoRetry).
6. ✅ Test tay §2 với sandbox dev đối tác (2026-09-14) qua Scalar `https://api-dev-erp.innvie.vn` (spec thật ở `/openapi/v1.json`, path `/scalar/...` chỉ là SPA shell không dùng được). Phát hiện + fix 2 bug trong `Dgas3ApiClient.cs` so với spec thật: (1) thiếu hẳn header `X-Tenant-Id` (required) — đã thêm, đọc từ `AppSettings:Dgas3TenantId` mới; (2) parse response sai vị trí — payload thật nằm trong `data.{transportOrderId,transportOrderCode,outcome}`, không phải root — đã sửa `ResponseEnvelopeDto`. Auth xác nhận đúng là header `X-API-Key` (không phải `Authorization: Bearer` như security scheme khai trong spec — thử Bearer bị 401, đổi qua X-API-Key thông ngay). Đã điền tạm `appsettings.json`: `Dgas3ApiKey=sk-transport-erp-abc` (đối tác cho để test), `Dgas3TenantId=362387869790208` (giá trị "default" trong spec — **CHƯA xác nhận có phải Tenant ID cố định của Delta hay chỉ mẫu**). Build 0 lỗi.
   - Test full payload thật (`VT2609/2215`, driver `45`, biển `29C-522.94`, `23.10` lít) → vẫn 404 `Dispatch.TransportOrderCompletion.ReferenceNotFound` — do driver/xe này CHƯA được seed trong tenant dev bên TransportERP (endpoint `hrm/v1/drivers/options`/`fleet/v1/vehicles/options` để tự kiểm đòi Bearer JWT nội bộ của họ, API key mình không có quyền đọc).
   - **PAUSE — chờ Innvie/TransportERP phản hồi**: (a) xác nhận Tenant ID `362387869790208` có cố định cho Delta không, (b) seed driver+xe mẫu (hoặc cho biết mã có sẵn) để test ra case "Created" thành công. Chưa cắm Đợt 2 (trigger tại 4 điểm chốt) — chờ đối tác trước.
...
4. ✅ 3/4 hàm "gửi khi chốt" đã code (2026-09-13, CHƯA cắm trigger, chỉ tạo hàm gọi tay/test được) — build 0 lỗi:
...
5. ⬜ Đợt 2: cắm 4 lời gọi trên vào đúng 4 điểm "chốt" thật (rà kỹ, tránh sót như bài học `WorkflowController.PromoteFromDraft` 2026-08-20) — bọc try/catch riêng, không rollback nghiệp vụ chốt nếu gọi dGas3 lỗi. Đợt 3: FE màn theo dõi mới thay `modal-fuel-summary` (thêm filter SentStatus đúng lúc này, xem gotcha BValue/TValue) → ẩn menu module dầu cũ (giữ dữ liệu lịch sử) → test E2E 2 chiều với đối tác → go-live cắt hẳn IGAS.
...
8. ✅ 2026-09-15: đổi `driverCode` gửi/nhận = `Employee.EmployeeCode` (không phải `DriverId`) — code xong (BE build 0 lỗi), CHƯA commit. ⬜ `Migration_FuelDgas3_DriverCode_20260915.sql` CHỜ ANH CHẠY — ⚠ PHẢI TRƯỚC khi deploy BE (5 SP nhận thêm `@DriverCode`, thiếu cột/param → lỗi "too many arguments"); chạy TRƯỚC `Migration_FuelDgas3_GetById_20260913.sql` (đã sửa trực tiếp thêm cột `DriverCode` vào SELECT, file đó cũng chưa chạy). Đã phát hiện + sửa luôn 2 chỗ trong `GaragesController` (§3 `Completions`, §4 `SearchTransportOrderCompletions`) đang `int.TryParse(driverCode)` — cùng lỗi với quyết định cũ. Chi tiết: `done.md` mục "dGas3 — đổi driverCode...".
