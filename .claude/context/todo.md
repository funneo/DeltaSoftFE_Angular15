...
1. ✅ Kế hoạch duyệt (2026-09-07). ✅ SQL đã soạn: `NewAPI/Migration_FuelDgas3_Integration_20260907.sql` — 3 bảng (`Tbl_FuelDgas3Outbound`/`Completion`/`CompletionDetail`) + 1 TVP `TypeFuelDgas3CompletionDetail` + 7 SP (`SP_FuelDgas3Outbound_GetBySourceRef`/`_Create`/`_UpdateResult`/`_GetFailedForRetry`/`_Search` + `SP_FuelDgas3Completion_GetByCompletionId`/`_Create`). 100% ADDITIVE, không đụng object cũ. ⬜ **Anh review + chạy** (login `delta.erp`).
2. ⬜ File SQL riêng CHƯA soạn: `Migration_FuelDgas3_Grant_*.sql` — Function + quyền màn "Theo dõi tích hợp dGas3" (chờ anh chốt FunctionCode, vd F047).
3. ⬜ Sau khi SQL chạy: BE (model/repo + adapter HttpClient `IDgas3ApiClient` gọi §2 + 2 controller `Dgas3IntegrationController` external §3/§4 + `FuelDgas3MonitorController` JWT + BackgroundService retry) → cắm trigger 4 điểm (rà kỹ cả 4 nguồn, tránh sót như bài học `WorkflowController.PromoteFromDraft` 2026-08-20) → FE màn theo dõi mới thay `modal-fuel-summary` → ẩn menu các module dầu cũ (giữ dữ liệu lịch sử, không xoá) → test E2E 2 chiều với đối tác → go-live cắt hẳn IGAS.
4. ⬜ Việc cần chốt cùng đối tác (ngoài phạm vi quyết định nội bộ): tên header xác thực + path cuối cùng cho §3/§4, danh sách enum đầy đủ của `fulfillmentMethod`.
