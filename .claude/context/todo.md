...
## ★★ Tích hợp dGas3/TransportERP thay IGAS (toàn bộ quản lý dầu) — KẾ HOẠCH XONG, CHỜ ANH DUYỆT rồi soạn SQL (2026-09-04, CHƯA CODE, CHƯA SQL)
...
1. ⬜ Anh duyệt kế hoạch (file trên) → em soạn file `.sql` (3 bảng + 7 SP) trình duyệt tiếp.
2. ⬜ Sau khi SQL chạy: BE (model/repo/adapter HttpClient gọi §2 + 2 controller mới `Dgas3IntegrationController`/`FuelDgas3MonitorController`) → cắm trigger 4 điểm (rà kỹ cả 4 nguồn, tránh sót như bài học `WorkflowController.PromoteFromDraft` hôm 2026-08-20) → FE màn theo dõi mới thay `modal-fuel-summary` → ẩn menu các module cũ (giữ dữ liệu lịch sử, không xoá) → test E2E 2 chiều với đối tác → go-live cắt hẳn IGAS.
3. ⬜ Việc cần chốt cùng đối tác (ngoài phạm vi quyết định nội bộ): tên header xác thực + path cuối cùng cho §3/§4, danh sách enum đầy đủ của `fulfillmentMethod`.
