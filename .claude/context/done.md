...
- **P0 SQL (chờ chạy)** [Migration_DraftSite_ShippingTask_20260720.sql](../../../NewAPI/Migration_DraftSite_ShippingTask_20260720.sql): DROP+CREATE `draft.SP_DraftEntries_Insert` — thân VERBATIM (đã đối chiếu OBJECT_DEFINITION live = khớp Phase0, KHÔNG bị sửa tay), delta duy nhất `@DraftType NOT IN (...,'ShippingTask')`. Verify DB: bảng `DraftEntries` KHÔNG có CHECK constraint, cột VARCHAR(20) đủ; `draft_app` có EXECUTE cấp SCHEMA::draft ⇒ DROP+CREATE không mất quyền. 5 SP còn lại + `dbo.SP_DraftEntries_GetForErp_GetPaging` generic theo @DraftType → không đụng. ⚠ Chạy TRƯỚC khi restart DraftAPI với appsettings mới.
...
- **CÒN LÀM**: P4 ERP (list `shipping-task-opman` trộn nháp + reuse modal CS `viewDraft()`/nền vàng + `ShippingTaskController.AddFromDraft` clone [ShipmentController.cs:476]) · P5 duyệt hàng loạt (Cách A: FE lặp `AddFromDraft` tuần tự + checkbox chọn nhiều, KHÔNG "Duyệt tất cả"). `PromotedRefNo` đề xuất = `ReferCode` (chờ anh xác nhận). ⬜ deploy: **build ERP API** (code Step=0 + appsettings) + **`ng build` draft-web** + copy appsettings DraftAPI & restart. (P0 SQL đã chạy.)
