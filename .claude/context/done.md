...
- **⬜ CÒN**: anh `ng serve` xem lại → `ng build` production + deploy. Test thật: mở lệnh đã có đủ 2 mốc t/g → bấm Check EUP ở cả list lẫn modal → đối chiếu đúng trạm/tiền; địa chỉ + ghi chú dài xuống dòng gọn, không tràn cột. Bổ sung nốt nhánh keyword của `SP_DispatchOrderFCL_GetAll` khi rảnh.

## FCL legacy — modal MỚI "Lịch sử lệnh" (lý do từ chối + timeline) — BE+FE, build 0 err, ĐÃ COMMIT+PUSH FE (BE NewAPI còn 1 file chờ anh tự commit) — 2026-08-06/07
...
- **⬜ CÒN**: FE (web-app-update) đã commit+push main. **BE (NewAPI) `DispatchOrderFCLHistory.cs` vẫn đang CHƯA COMMIT** (repo khác, anh tự commit) → **tắt API + `dotnet publish`** (Model C# đổi) → `ng build` production deploy FE → test: mở lịch sử 1 lệnh từng bị từ chối ở bước chốt lệnh → cột "Bước" hiện đúng "Từ chối chốt lệnh", "Người thực hiện" có tên.
...
- **⬜ CÒN**: `dotnet build -t:Compile` 0 Error (đã verify) nhưng **CHƯA COMMIT**. Anh review diff → commit+push → tắt API → `dotnet publish`.
...
- **⬜ CÒN**: anh review + commit fix attach 2026-08-06 → chạy SQL FeePerLineV2 (⚠ TRƯỚC khi deploy) → tắt API → `dotnet publish` → mobile đổi handler (POST multipart CreateFeeV2/UpdateFeeV2 kèm ảnh; DeleteFeeV2; driverUpdate `skipFeeMerge=1`; đọc `data.Id`). Web KHÔNG đổi.
