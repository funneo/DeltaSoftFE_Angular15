...
## Fix double dữ liệu khi Duyệt nháp ERP (race condition ở AddFromDraft) — BE-only, build 0 lỗi, CHỜ chạy SQL + tắt API build/publish — 2026-08-19
...
- **CÒN**: anh chạy SQL (⚠ TRƯỚC khi deploy BE, vì BE mới gọi thẳng 2 SP mới) → tắt API (khóa DLL) → build/publish → test: 2 tab/2 tài khoản duyệt trùng 1 nháp xác nhận chỉ 1 bản ghi thật; test "Duyệt nhiều" bình thường không regression; test case tạo lỗi giữa chừng → nháp trả về `Draft` (không kẹt `Promoting`).
...
- **CÒN**: anh chạy file SQL (login delta.erp) → `ng build` + deploy FE → test: mở "Chốt dầu phương tiện" 1 xe có phiếu Mua dầu ngoài Status=3 trong tháng, xác nhận nhóm mới hiện đúng số lít + Duyệt xong phiếu nguồn được đánh dấu `IsFuelClosing=1`.

## HTTP Response Compression (gzip/brotli) cho toàn bộ API — BE-only, build 0 lỗi, CHỜ tắt API build/publish + test — 2026-08-18
...
- **CÒN**: anh tắt API đang chạy (khóa DLL) → build/publish → test: DevTools web xem header `Content-Encoding` + kích thước response trước/sau; gọi thử API từ app Flutter xác nhận đọc dữ liệu bình thường; thử luôn Innvie/EUP (client ngoài dùng `Api-Key`, không phải JWT — cùng cơ chế content-negotiation nên lý thuyết an toàn, nhưng nên xác nhận thực tế 1 lần).
