...
## dGas3 — đổi driverCode gửi/nhận = Employee.EmployeeCode (không phải DriverId) — code xong, CHỜ chạy SQL + commit — 2026-09-15
...
- **SQL mới** `NewAPI/Migration_FuelDgas3_DriverCode_20260915.sql` (additive, CHƯA CHẠY): `Tbl_FuelDgas3Outbound`+`Tbl_FuelDgas3Completion` thêm cột `DriverCode NVARCHAR(50)` (giữ nguyên `DriverId INT` để BE liên kết nội bộ) + 2 index mới; recreate 5 SP (`SP_FuelDgas3Outbound_Create/GetBySourceRef/GetFailedForRetry/Search`, `SP_FuelDgas3Completion_Create`) thêm `@DriverCode`/cột `DriverCode`. Đã sửa trực tiếp `Migration_FuelDgas3_GetById_20260913.sql` (chưa chạy nên sửa thẳng, không patch thêm) để `SELECT` có `DriverCode`.
- **BE đã sửa** (build 0 lỗi, CHƯA commit): model `FuelDgas3Outbound`/`FuelDgas3Completion` +`DriverCode`; `IFuelDgas3Outbound.SearchAsync` +tham số `driverCode` cuối (optional); 2 repository + `FuelDgas3MonitorController.Resend` đổi `row.DriverId?.ToString()`→`row.DriverCode`; 3 service sync (`Dgas3FclSyncService`/`Dgas3DispatchOrderSyncService`/`Dgas3AdditionalFeeSyncService`) thêm `e.EmployeeCode AS DriverCode` vào SourceSql + chặn gửi (return `Success=false`) khi `DriverCode` rỗng + payload gửi `row.DriverCode`; `GaragesController.Completions` (§3) lưu thẳng `body.DriverCode` (bỏ parse int); `SearchTransportOrderCompletions` (§4) lọc theo `@DriverCode` string, trả `x.DriverCode`.
- **CÒN**: chưa chạy `Migration_FuelDgas3_DriverCode_20260915.sql` (PHẢI chạy TRƯỚC khi deploy BE vì SP có `@DriverCode` mới — nếu không sẽ lỗi "too many arguments"); chưa chạy `Migration_FuelDgas3_GetById_20260913.sql` (chạy SAU file DriverCode vì phụ thuộc cột mới); chưa commit git; trigger Đợt 2 vẫn chưa cắm.
...
- **CÒN**: chưa commit git; chưa cắm trigger (Đợt 2); `Migration_FuelDgas3_GetById_20260913.sql` (SP additive `SP_FuelDgas3Outbound_GetById` cho nút "Gửi lại" xử lý dòng Failed-NoRetry) — soạn xong, CHỜ anh chạy.
