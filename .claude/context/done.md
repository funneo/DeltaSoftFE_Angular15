...
- **✅ ĐÃ chạy SQL + `ng build`/deploy FE** (anh xác nhận 2026-08-20). **CÒN**: test — mở "Chốt dầu phương tiện" 1 xe có phiếu Mua dầu ngoài Status=3 trong tháng, xác nhận nhóm mới hiện đúng số lít + Duyệt xong phiếu nguồn được đánh dấu `IsFuelClosing=1`.

## HTTP Response Compression (gzip/brotli) cho toàn bộ API — ✅ ĐÃ build/publish (anh xác nhận 2026-08-20), CHỜ TEST — 2026-08-18
...
- **✅ ĐÃ tắt API build/publish** (anh xác nhận 2026-08-20). **CÒN**: test — DevTools web xem header `Content-Encoding` + kích thước response trước/sau; gọi thử API từ app Flutter xác nhận đọc dữ liệu bình thường; thử luôn Innvie/EUP (client ngoài dùng `Api-Key`, không phải JWT — cùng cơ chế content-negotiation nên lý thuyết an toàn, nhưng nên xác nhận thực tế 1 lần).
