...
## FCL legacy — modal MỚI "Lịch sử lệnh" (lý do từ chối + timeline) — FE-only, ng build 0 err, CHƯA COMMIT — 2026-08-06
Anh hỏi lý do từ chối ở bước chốt lệnh cuối cùng (lệnh FCL **cũ**, khác v2) ghi ở đâu để mọi người xem được. Truy ra: `SP_DispatchOrderFCL_UpdateState` (legacy) đã tự `INSERT INTO DispatchOrderFCLHistory VALUES (@RefNo,@Step,@UserId,@Feedback,GETDATE())` mỗi lần đổi trạng thái (kể cả từ chối) — comment lấy từ `Migration_FCLStatusLog_20260710.sql`. BE đã đọc lại đủ qua `GetByRefNo`/`GetByRefNoWithTOAsync` (`entity.ListHistory`), nhưng **FE trước giờ KHÔNG hiển thị ở đâu cả** (model FE thiếu field, cả modal tạo/sửa lẫn modal thực hiện đều không bind) — dữ liệu có ghi nhận đầy đủ (ai/khi nào/lý do gì) nhưng không ai xem lại được.
- **Không cần SQL/BE mới** — API `GetByRefNo` sẵn đã trả `listHistory`, chỉ là FE chưa đọc.
- **Model** [dispatch-order-fcl.ts](../../src/app/shared/models/fcl/dispatch-order-fcl.ts): +`listHistory?: DispatchOrderFclHistory[]` + interface mới `DispatchOrderFclHistory{id,refNo,employeeId,employeeFullName,notes,createdDate}`.
- **Modal mới** `modal-fcl-history` (`src/app/shared/components/transports/modal-fcl-history/`): `.component.ts/.html/.module.ts` theo đúng khuôn skill `deltasoft-modals` (ngDraggable+[handle], lazy-mount, `show(refNo)` gọi lại `dispatchOrderFclService.getDetail(refNo)` rồi lấy `data.listHistory`, sort **mới nhất lên trên**). Bảng 3 cột: Thời gian / Người thực hiện / Nội dung-Lý do.
- **Gắn nút "Lịch sử"** (cạnh nút "Đính kèm" có sẵn, *ngIf="!flagNew") vào **cả 2** modal legacy: `modal-dispatch-order-fcl` (tạo/sửa) và `modal-perform-fcl` (thực hiện) — thêm field `viewFclHistory`/`ViewChild`/`viewHistory()`/`closeModalFclHistory()` theo đúng pattern `attachFile()` đã có sẵn trong chính 2 file đó (copy khuôn, không đổi logic cũ). 2 module tương ứng +import `ModalFclHistoryModule`.
- **Verify**: `npx tsc --noEmit` 6 lỗi đều pre-existing (spec sai tên service + driver-fuel-closing, không liên quan) + `npx ng build` (default config) **0 lỗi mới**, Hash `66c7f59779c7eeaa`.
- **⬜ CÒN**: anh review diff → commit+push → `ng build` production deploy. Không cần redeploy BE/chạy SQL.
