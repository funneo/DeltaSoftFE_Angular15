...
- **BE** ([GaragesController.cs](../../../NewAPI/API/Controllers/CustomerCommunicate/GarageInnvie/GaragesController.cs)): thêm region **Nhân viên** + endpoint `POST /api/Garages/GetEmployees` (`[AllowAnonymous]` + check header `Api-Key` giống `GetOtherCategories`). Dùng `_employee.GetbyAll()` (đã inject sẵn, gọi `SP_Employee_GetbyAll`), lọc `Status==true`, trả projection gọn: `Id, EmployeeCode, EmployeeFullName, BranchId, BranchName, DepartmentName, TitleName, Telephone, Email`. Bọc `ResponseGarageAPI{isAcknowledged,Errors,Result}` đồng nhất các endpoint garage khác.
- **Anh cần**: deploy API → app garage gọi `GetEmployees` (header `Api-Key`) lấy list NV. Nếu cần thêm field (ImagePath/AccountNumber) hoặc lấy cả NV nghỉ → báo chỉnh projection/bỏ filter.
