...
## FCL v2 — Cờ "Tránh trạm" (DispatchOrderFCLEtc.IsPassed) bị xóa khi điều vận lưu lệnh — SQL soạn xong, CHỜ anh chạy — 2026-09-10
...
- **Script khôi phục** `NewAPI/Fix_FCL_ResetRejectedClosing_ToStatus2_20260910.sql` — đưa các lệnh v2 **đang bị từ chối chốt** (dòng log gần nhất `ActionType=7`, Status=3) về Status=2 để lái xe cập nhật lại; `ActionBy` lấy từ lịch sử chính lệnh; ghi 1 dòng `Tbl_DispatchOrderFCLStatusLog` truy vết; transaction có `@Commit` (mặc định 0 = dry-run). Phạm vi tính đến 2026-09-10: **22 lệnh**.
...
- **CÒN**: (1) anh chạy `Migration_FCL_WithTO_EtcIsPassed_20260910.sql`; (2) chạy `Fix_FCL_ResetRejectedClosing_ToStatus2_20260910.sql` (`@Commit=0` xem trước → `@Commit=1` ghi thật); (3) lái xe/điều vận vào lại từng lệnh tick "Tránh trạm" → Duyệt B1 → kiểm tra cờ không còn bị xóa. Riêng FCL-VT260909/0068 đang Status=5 (đã Duyệt B1 lại) → nằm ngoài 22 lệnh, cần `UPDATE DispatchOrderFCLEtc SET IsPassed=1 WHERE Id IN (93275,93276)` nếu xác nhận xe tránh trạm thật.
