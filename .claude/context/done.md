...
### 2. Báo cáo CP03 (report-cp03-filter) — điều tra + fix "chênh lệch ảo" — SQL ĐÃ CHẠY (2026-09-22)
...
### 3. FCL v2 — cho phép điều vận sửa lại lệnh bị tài xế "Từ chối nhận" — SQL ĐÃ CHẠY (2026-09-22), FE ĐÃ SỬA
...
- **Xác minh BE vô can**: đọc `DispatchOrderFCLRepository.CreateWithTOAsync`/`UpdateWithTOAsync` — `ListEtcPenalty` chỉ được gán khi GET (`GetByRefNoWithTOAsync`), KHÔNG hề đọc ở 2 hàm ghi trên. Vé ETC ngoài kế hoạch có CRUD riêng (`Add/Update/DeleteEtcPenalty`, auto-save từng dòng qua `saveEtcPenaltyRow`) — field này gửi lên `UpdateWithTO`/`CreateWithTO` là thừa, chỉ gây lỗi.
- **Fix** (`dispatch-order-fcl.service.ts`, FE-only): bỏ hẳn `listEtcPenalty` khỏi payload ở `createWithTo`/`updateWithTo` (`{ ...entity, listEtcPenalty: undefined }`) thay vì convert định dạng — vì BE không đọc field này ở 2 endpoint đó nên không cần gửi.
- **CÒN**: cần `ng build` để lên hiệu lực; BE không đổi gì, không cần deploy lại API.

## dGas3 — đổi driverCode gửi/nhận = Employee.EmployeeCode (không phải DriverId) — SQL đã chạy, test sandbox OK, ĐÃ COMMIT+PUSH — 2026-09-15
Anh Cường yêu cầu: trigger dGas3 khi chốt lệnh/lệnh FCL/chốt bổ sung chi phí phải gửi **mã nhân viên** (`Employee.EmployeeCode`, vd "NV001"), KHÔNG gửi `DriverId` nội bộ như quyết định cũ 2026-09-04. Phát hiện thêm: §3 (webhook nhận kết quả) + §4 (đối tác tra cứu lại) **đã có code sẵn** trong `GaragesController` (Đợt 1, 2026-09-13/14) và đang `int.TryParse(body.DriverCode)` — cùng lỗi, đã sửa luôn.
- **Quyết định qua AskUserQuestion**: (1) driver thiếu EmployeeCode → **chặn không gửi, ghi lỗi** (không phải gửi rỗng); (2) `SP_FuelDgas3Outbound_Search` giữ **cả 2 filter song song** `@DriverId` (nội bộ) + `@DriverCode` (mới, đối tác dùng).
- **SQL** `NewAPI/Migration_FuelDgas3_DriverCode_20260915.sql` (additive, **ĐÃ CHẠY 2026-09-15**): `Tbl_FuelDgas3Outbound`+`Tbl_FuelDgas3Completion` thêm cột `DriverCode NVARCHAR(50)` (giữ nguyên `DriverId INT` để BE liên kết nội bộ) + 2 index mới; recreate 5 SP (`SP_FuelDgas3Outbound_Create/GetBySourceRef/GetFailedForRetry/Search`, `SP_FuelDgas3Completion_Create`) thêm `@DriverCode`/cột `DriverCode`. `Migration_FuelDgas3_GetById_20260913.sql` cũng đã chạy để `SELECT` có `DriverCode`.
- **BE đã sửa** (build 0 lỗi, **ĐÃ COMMIT+PUSH** repo NewAPI `127352e`): model `FuelDgas3Outbound`/`FuelDgas3Completion` +`DriverCode`; `IFuelDgas3Outbound.SearchAsync` +tham số `driverCode` cuối (optional); 2 repository + `FuelDgas3MonitorController.Resend` đổi `row.DriverId?.ToString()`→`row.DriverCode`; 3 service sync (`Dgas3FclSyncService`/`Dgas3DispatchOrderSyncService`/`Dgas3AdditionalFeeSyncService`) thêm `e.EmployeeCode AS DriverCode` vào SourceSql + chặn gửi (return `Success=false`) khi `DriverCode` rỗng + payload gửi `row.DriverCode`; `GaragesController.Completions` (§3) lưu thẳng `body.DriverCode` (bỏ parse int); `SearchTransportOrderCompletions` (§4) lọc theo `@DriverCode` string, trả `x.DriverCode`.
- **Test thật với sandbox dev Innvie (2026-09-15)**: thêm action tạm `FuelDgas3MonitorController.TestSend` (gọi thẳng `IDgas3ApiClient`, không ghi DB) để test tay — gửi `driverCode="NV046"` (data thật anh cung cấp) → **HTTP 200, outcome=created**. Khác hẳn lần test trước (2026-09-13, gửi DriverId nội bộ "45" → 404 `ReferenceNotFound`) — xác nhận driver/xe đã có sẵn bên tenant dev Innvie theo mã NV thật, không phải chờ đối tác seed thêm như ghi nhận cũ.
- **Pilot Đợt 2 (2026-09-15)**: cắm `SyncDispatchOrderClosedAsync` vào `DispatchOrderController.UpdateState` khi `Status=8`("chốt lệnh")/`11`("chốt dừng lệnh") — try/catch riêng, không rollback nghiệp vụ chốt nếu dGas3 lỗi. Đây là **PILOT 1/4 điểm** (chỉ DispatchOrder cũ) — 3 điểm còn lại (FCL legacy/v2, AdditionalFee) CHƯA cắm, chờ pilot này chạy đúng trên môi trường thật.
- **CÒN**: anh cần tắt API đang chạy → build/publish lại (SQL đã chạy nên không còn phụ thuộc); test thật bằng cách chốt 1 lệnh DispatchOrder cũ (status→8/11) rồi xem kết quả ở màn F047; sau đó mới cắm nốt 3 điểm còn lại (Đợt 2 phần 2); gỡ action tạm `TestSend` khi Đợt 2 xong hết.
...
## dGas3/TransportERP Đợt 1 BE — model/repo/3 hàm sync/adapter/controller — build+publish 0 lỗi, ĐÃ COMMIT+PUSH, pilot Đợt 2 cắm 1/4 điểm (xem mục driverCode=EmployeeCode ở trên) — 2026-09-13/14
Thực hiện Đợt 1 theo kế hoạch đã duyệt (xem todo.md mục dGas3). Toàn bộ ADDITIVE, không sửa code/SP cũ.
- **Model/Interface/Repository** (`Models|Interfaces|Repositories/CustomerCommunicate/Dgas3/`): `FuelDgas3Outbound`/`FuelDgas3Completion` + `IFuelDgas3Outbound`(`GetBySourceRef/GetById/Create/UpdateResult/GetFailedForRetry/Search`)/`IFuelDgas3Completion`(`GetByCompletionId/Create`), map đúng 7 SP đã chạy từ `Migration_FuelDgas3_Integration_20260907.sql`.
- **Adapter outbound** `Services/Dgas3ApiClient.cs` (`IDgas3ApiClient`) — HttpClient POST `{Dgas3BaseUrl}/api/dispatch/v1/integrations/delta-erp/transport-order-completions` (path §2 ĐÚNG theo docx đối tác), header `X-API-Key`+`Idempotency-Key`, map HTTP status → `CanRetry` (408/429/5xx). `appsettings.json` thêm `Dgas3BaseUrl`/`Dgas3ApiKey` (rỗng, điền khi có từ đối tác).
- **3 hàm "gửi khi chốt"** (gọi tay/test được, KHÔNG cắm trigger) — công thức xác minh trực tiếp từ SP đang chạy thật, không đoán:
  - `Dgas3FclSyncService.SyncFclClosedAsync(refNo, isLegacy)` — SourceType 3 (FCL legacy)/4 (v2), dùng chung vì cả 2 cùng gọi `SP_DispatchOrderFCL_Chotdau`. `EligibleFuelLiters = Tongdau + GeneratorFuelAmount` (đã hỏi anh xác nhận 2026-09-13 — KHÔNG cộng thêm dòng phí FeeId=666 vì luồng TopUp đang bị bỏ theo scope dGas3). Bỏ qua lệnh thầu phụ + khi liters≤0.
  - `Dgas3DispatchOrderSyncService.SyncDispatchOrderClosedAsync(refNo)` — SourceType 1 (DispatchOrder cũ). Đọc `SP_DispatchOrder_UpdateState` (@Type=8/11) xác nhận: `EligibleFuelLiters = DispatchOrderFee.Quantity WHERE FeeId=666`. KHÔNG loại trừ thầu phụ (SP gốc không loại trừ ở nhánh này).
  - `Dgas3AdditionalFeeSyncService.SyncAdditionalFeeAcceptedAsync(refNo)` — SourceType 2 (AdditionalFee). Đọc `SP_DispatchOrderAdditionalFee_UpdateState` (@Status=3) xác nhận: `EligibleFuelLiters = DispatchOrderAdditionalFeeDetailed.Quantity WHERE FeeId=666`, chỉ khi `Type<>1` ("người lập phiếu" không có dầu) và không thầu phụ (join `DispatchOrderRefNo`→`DispatchOrder` cha lấy `IsSubcontractors`/`DriverId`/`BranchId`).
  - Cả 3: quy đổi giờ VN→UTC cố định `-7h` (không dựa `TimeZoneInfo` server, tránh lệch nếu server set UTC); `IdempotencyKey = Guid.NewGuid()` lúc tạo dòng (anh xác nhận không cần compose theo format riêng).
- **Controller nội bộ** `FuelDgas3MonitorController` (JWT, `[ClaimRequirement(FunctionCode.F047, ...)]`) — `GetPaging`/`Resend`. Đợt 1 CHƯA lọc `SentStatus` trong `GetPaging` (gotcha `FromBodyBase.TValue` non-nullable, dời sang Đợt 3).
- **Controller đối tác gọi vào — GỘP CHUNG vào `GaragesController`** (quyết định 2026-09-13 "chung của Innvie"): ban đầu tạo `Dgas3IntegrationController` riêng, sau đổi sang thêm 2 action `completions`/`transport-order-completions/search` thẳng vào `GaragesController` (`Controllers/CustomerCommunicate/GarageInnvie/`) vì cùng dùng chung key incoming `AppSettings:ApiKey` với Innvie/Garage. Đã xóa `Dgas3IntegrationController.cs`.
  - Path `completions` (§3) dùng CHUẨN CỦA MÌNH (`api/Garages/completions`) — đối chiếu docx `tai-lieu-ket-noi-delta-erp-transport-order-dgas3.docx` thì TransportERP đề xuất path khác (`/api/integrations/v1/transport-erp/dgas3-completions`), nhưng anh Cường chốt KHÔNG cần theo path đối tác đề xuất, báo lại path của mình khi chốt kết nối thật.
  - Path §2 (outbound, ERP gọi ra) đã đúng docx từ đầu — giữ nguyên (path CỦA ĐỐI TÁC, bắt buộc gọi đúng).
  - `transport-order-completions/search` (§4 tra cứu) — phát hiện docx đối tác **CHỈ có §1-§3, không có §4** — đây là API tự thêm nội bộ trước đó, path/response CHƯA chốt với đối tác.
- **Verify**: `dotnet build` 0 Error nhiều lần; `dotnet publish -c Release` cũng 0 Error (2026-09-14, xác nhận build production sạch).
- **CÒN**: xem mục "dGas3 — đổi driverCode..." ở trên cho trạng thái mới nhất (SQL đã chạy, commit/push xong, pilot Đợt 2 1/4 điểm).

## FCL v2 — Admin bypass CHỐT LỆNH + điều tra trang.nguyen — xem chi tiết mục "✅ FCL v2 — Admin không thấy nút CHỐT LỆNH" ở todo.md — 2026-09-12

## FCL v2 — Reset 3 lệnh Status=6 mất cờ Tránh trạm về Status=2 — ĐÃ XÁC NHẬN CHẠY XONG (2026-09-14)
`Fix_FCL_Reset3ChotXongLoiIsPassed_ToStatus2_20260912.sql` (3 lệnh `FCL-VT260907/0009`,`/0010`,`/0068`, thuộc nhóm 8 lệnh Status=6 mất cờ Tránh trạm trước ngày fix — xem todo.md mục IsPassed). Ban đầu (2026-09-12→13) anh báo đã chạy nhưng verify DB 2 lần liên tiếp vẫn thấy Status=6/IsPassed=0/không log mới — **có mâu thuẫn thật giữa lời anh nói và dữ liệu**, đã báo lại minh bạch thay vì tự suy đoán bên nào đúng. Verify lại 2026-09-14: xác nhận **đã chạy thật** — cả 3 lệnh về Status=2, `ClosingBy`/`ClosingDate=NULL`, có log `ActionType=7, ToStatus=2, CreatedDate 2026-09-14 11:06:25`.

## FCL v2 — Hiện đầy đủ điểm đi/điểm đến ở bảng Cung đường — FE-only, CHỜ commit+deploy — 2026-09-13
Xem chi tiết mục "▶ FCL v2 — Hiện đầy đủ điểm đi/điểm đến..." ở todo.md. `ng build --configuration production` đã verify sạch tại máy dev 2026-09-14 (exit 0, `dist/web-app/` sinh đầy đủ) — máy dev không có đường lên server thật, anh tự đẩy dist lên.

## FCL v2 — Tách VAT 8% cho vé ETC (trạm thu phí tự động từ cung đường) — FE ĐÃ SỬA (tsc 0 lỗi mới), SQL soạn xong CHỜ anh chạy — 2026-09-11
Anh hỏi rà soát: giá vé Vietmap trả về (`route-tolls` → `t.price`) chỉ có 1 field giá duy nhất, KHÔNG có cấu trúc trước-VAT/VAT/tổng. FE cũ (`modal-dispatch-order-fcl-v2.component.ts`, `_syncEtcFromSegment`/`_recomputeAutoEtcPrices`) gán thẳng `Cost = price`, `Vat = 0` (hardcode), `TotalCost = Cost` → mọi dòng ETC tự động từ cung đường (Vietmap tự thêm hoặc thêm tay từ danh mục Trạm) đều "Trước VAT = Sau VAT, VAT = 0" (sai). Anh chốt: VAT = 8%, giá Vietmap trả là **Sau VAT** (giá thực trả tại trạm, không đổi), tách ngược ra Trước VAT — **không tính xuôi** (tránh lệch số Sau VAT dùng để trừ lương lái xe).
- **Công thức đã chốt**: `VAT = floor(SauVAT × 8/108 + 0.5)` (làm tròn 0.5 lên 1) → `TrướcVAT = SauVAT − VAT`. Tính VAT trước, TotalCost giữ nguyên tuyệt đối (không dùng `Cost×1.08` vì có thể lệch vài đồng so với giá trạm thật).
- **FE đã sửa** ([modal-dispatch-order-fcl-v2.component.ts:1887-1930](../../../src/app/shared/components/transports/modal-dispatch-order-fcl-v2/modal-dispatch-order-fcl-v2.component.ts#L1887-L1930)): thêm helper `_splitVat()` dùng chung; áp vào `_syncEtcFromSegment` (mọi lần đồng bộ trạm từ cung đường vào bảng ETC — cả trạm Vietmap tự thêm và trạm thêm tay từ danh mục qua `confirmAddManualStation`) + `_recomputeAutoEtcPrices` (đổi loại xe → tính lại giá). Dòng "Thêm trạm" tay trống (`newEtc`/`updateTotalCost`, user tự nhập Cost+VAT rồi cộng ra Tổng) GIỮ NGUYÊN — không thuộc phạm vi "chọn vé từ cung qua vé ETC" anh hỏi. `npx tsc --noEmit` không phát sinh lỗi ở file này.
- **SQL dữ liệu cũ** `NewAPI/Migration_DispatchOrderFCLEtc_Vat8Percent_20260911.sql` — UPDATE tách lại Vat/Cost cho các dòng đã lưu trước đó, **TotalCost giữ nguyên 100%** (không đổi số tiền đã trừ). Đã verify DB (chỉ SELECT, KHÔNG sửa): lệnh FCL v2 (`IsLegacy=0`) có 589 dòng ETC — **585 dòng đang Vat=0 (bug)** sẽ được tách lại, 4 dòng còn lại (1 lệnh test `FCL-VT260909/0032`, đã có Vat≠0 theo công thức khác) tự động KHÔNG bị đụng (`WHERE Vat=0`). **31.392 dòng ETC của lệnh legacy (`IsLegacy=1`) hoàn toàn không đụng** — SP/luồng khác, đúng nguyên tắc không chạm legacy. Script có kèm block `[0] PREVIEW` (SELECT trước khi UPDATE) + `[2] Verify` (check `Cost+Vat=TotalCost` khớp hết, không còn dòng Vat=0 sót). Idempotent — chạy lại vô hại.
- **CÒN**: (1) anh chạy PREVIEW soát rồi chạy UPDATE trong `Migration_DispatchOrderFCLEtc_Vat8Percent_20260911.sql` (login delta.erp) — không cần deploy BE (SP không đổi, chỉ sửa data); (2) `ng build` + deploy FE; (3) test E2E: mở lệnh FCL v2 mới, cung có trạm 196.000đ → bảng ETC hiện Trước VAT ~181.481 / VAT ~14.519 / Tổng 196.000 (khớp giá trạm thật).

## FCL v2 — Cờ "Tránh trạm" (DispatchOrderFCLEtc.IsPassed) bị xóa khi điều vận lưu lệnh — ✅ ĐÃ CHẠY HẾT SQL + reset dữ liệu (2026-09-10), CHỜ ng build FE + test E2E — 2026-09-10
Lái xe tick "Tránh trạm" (trước gọi "Trốn vé", cột `DispatchOrderFCLEtc.IsPassed`) lưu qua đường lái xe (`SP_FCL_DriverUpdate_SaveFeeEtc` — `SET e.IsPassed`). Nhưng điều vận bấm **Duyệt B1** ở modal v2 gọi `updateWithTo` trước → `SP_DispatchOrderFCL_UpdateWithTO` đang `DELETE FROM DispatchOrderFCLEtc WHERE RefNo=@RefNo` + `INSERT` lại **không có cột IsPassed** → cờ về 0. Người chốt lệnh thấy "chưa tick tránh trạm" nên từ chối chốt → lệnh kẹt.
- **Bằng chứng DB** (lệnh FCL-VT260909/0068): Id các dòng `DispatchOrderFCLEtc` cấp mới tăng dần **theo `UpdatedDate` của lệnh**, không theo `CreatedDate` → mỗi lần điều vận lưu = xóa + chèn lại toàn bộ, Id churn. `Tbl_DispatchOrderFCLStatusLog`: Duyệt B1 (12:01) → Từ chối chốt (14:37, lý do "Chưa tick tránh trạm") → Duyệt B1 lại (15:03). Header `UpdatedDate=15:03:04` trùng lần Duyệt B1 cuối.
- **Fix** `NewAPI/Migration_FCL_WithTO_EtcIsPassed_20260910.sql` (3 phần):
  1. Index `IX_DispatchOrderFCLEtc_RefNo` (bảng đang là HEAP thuần, 31.911 dòng, không index nào → mọi thao tác theo RefNo quét full bảng). NONCLUSTERED + INCLUDE các cột hay đọc. Additive.
  2. `SP_DispatchOrderFCL_UpdateWithTO` — khối ETC đổi từ `DELETE`+`INSERT` sang **khuôn `SP_Payments_Update`**: đổ `@ListEtc` ra `#Etc` (có `NULLIF(Id,0)` + filtered index `IX_Etc_Id`), rồi UPDATE (join Id, scope `RefNo=@RefNo`, **KHÔNG có `IsPassed` trong SET**) → DELETE (anti-join, scope RefNo) → INSERT (Id null hoặc chưa tồn tại, `IsPassed=0`). Kết quả: điều vận lưu KHÔNG chạm `IsPassed`, Id ổn định, chỉ chạm ~2 dòng qua index.
  3. `SP_DispatchOrderFCL_CreateWithTO` — thêm `IsPassed` vào câu INSERT ETC (nhất quán; tạo mới hầu như luôn 0).
- **KHÔNG đụng**: SP legacy (`SP_DispatchOrderFCL_Create/_Update/_UpdateState`), đường lưu của lái xe. **Không cần redeploy BE** (repo đã gửi `x.Id` + `x.IsPassed` trong TVP `TypeDispatchOrderFCLEtcV2` sẵn). FE không đổi — phần hiển thị đã đúng (`[(ngModel)]="item.isPassed"`), lỗi chỉ ở dữ liệu bị SP xóa.
- **2 script khôi phục dữ liệu** (`ActionBy` lấy từ lịch sử chính lệnh; ghi 1 dòng `Tbl_DispatchOrderFCLStatusLog` truy vết `ActionType=7 → ToStatus=2`; transaction có `@Commit`, mặc định 0 = dry-run):
  - `NewAPI/Fix_FCL_ResetRejectedClosing_ToStatus2_20260910.sql` — lệnh v2 **đang bị từ chối chốt** (dòng log gần nhất `ActionType=7`, Status=3) → Status=2. **22 lệnh**.
  - `NewAPI/Fix_FCL_ResetChoChot_ToStatus2_20260910.sql` — lệnh v2 **đang Chờ chốt** (Status=5) mà có dòng ETC nhưng KHÔNG dòng nào `IsPassed=1` → Status=2. **16 lệnh** (20 lệnh Status=5 − 4 lệnh không có dòng ETC).
- **Memory** `reference_sp_child_grid_update_pattern` — ghi lại khuôn `SP_Payments_Update` (TVP→#temp có index → UPDATE/DELETE/INSERT tường minh, KHÔNG MERGE, scope parent id, guard cột do actor khác sở hữu).
- **✅ ĐÃ CHẠY HẾT (2026-09-10)**: `Migration_FCL_WithTO_EtcIsPassed_20260910.sql` (verify live: `_UpdateWithTO` có block `#Etc` UPDATE/DELETE/INSERT không đụng `IsPassed`, `_CreateWithTO` có `IsPassed`, index `IX_DispatchOrderFCLEtc_RefNo` tồn tại) + 2 script reset (`@Commit=1`). Phân bổ lệnh v2 sau reset: `1→136, 2→113, 3→38, 5→4, 6→45`. 16 lệnh Chờ chốt đã về Status=2 + 16 dòng log; 22 lệnh từ chối chốt đã reset trước đó (15 về 2, 6 lái xe hoàn thành lại có IsPassed đúng, 1 nằm trong nhóm Chờ chốt).
- **CÒN**: (1) `ng build` + deploy FE (fix IsPassed không cần FE, nhưng đổi nhãn "Trốn vé"→"Tránh trạm"/nút GPS-EUP/ẩn Km đầu-cuối cần); (2) test E2E — lấy 1 lệnh vừa reset (Status=2) → tick "Tránh trạm" → Hoàn thành (2→3) → điều vận Duyệt B1 (3→5) → mở lại: cờ vẫn còn ✅; chốt (5→6): cờ vẫn nguyên.

## FCL — rà soát Từ chối B1/Từ chối chốt cũ vs mới — XÁC NHẬN ĐÃ ĐÚNG, KHÔNG SỬA GÌ (quyết định cuối) — 2026-09-08
Anh yêu cầu rà lại luồng Từ chối B1/Từ chối chốt của lệnh FCL, đối chiếu cũ (legacy) vs mới (v2).
- **Lệnh MỚI (v2)** — đọc live definition `SP_DispatchOrderFCL_ChangeStatus` qua `OBJECT_DEFINITION`: đã đúng 100% (Từ chối B1: 3→2, Từ chối chốt: 5→3), tự chặn `IsLegacy=1` không cho chạy nhầm (redirect sang `SP_DispatchOrderFCL_UpdateState`). Không cần sửa gì.
- **Lệnh CŨ (legacy)** — phát hiện bug thật ở `SP_DispatchOrderFCL_UpdateState` (đọc live definition qua `OBJECT_DEFINITION`): khi `@IsRejection=1`, SP tự tính lại `@Status`, bỏ qua giá trị FE gửi. Đối chiếu FE [modal-dispatch-order-fcl.component.ts:1521-1552](../../../src/app/shared/components/transports/modal-dispatch-order-fcl/modal-dispatch-order-fcl.component.ts#L1521-L1552) — hàm `tuchoiChot()` (status=5) ý định về status=3, luôn gửi `TypeRejection=0`, nhưng SP không có nhánh riêng cho `@CurrentStatus=5` nên rơi vào nhánh `IIF(@TypeRejection=0,2,3)` (thiết kế gốc cho status=4) → trả SAI về status=2 thay vì 3.
- Đã soạn sẵn `ALTER PROCEDURE` fix 1 dòng cho SP cũ (`Migration_FCL_Legacy_UpdateState_RejectClosing_Fix_20260908.sql`), nhưng **anh chốt lại phạm vi: KHÔNG đụng SP cũ, chỉ giới hạn ở lệnh FCL MỚI** (đúng nguyên tắc SP cũ bất khả xâm phạm). File đã XÓA, KHÔNG áp dụng.
- **Kết luận: không cần sửa SQL/BE/FE gì cả** — v2 đã đúng sẵn từ đầu; bug ở legacy chấp nhận giữ nguyên, không sửa.

## FCL v2 — Vé ETC ngoài kế hoạch (trừ lương lái xe)

## FCL v2 — Vé ETC ngoài kế hoạch (trừ lương lái xe) — SQL soạn xong + BE/FE build 0 lỗi mới, CHỜ anh chạy SQL + deploy — 2026-09-08
Lái xe né trạm thu phí CHÍNH trên lộ trình lệnh nhưng vẫn đi qua 1 trạm PHỤ không thuộc lộ trình → tài khoản ETC công ty vẫn bị trừ tiền, khoản này KHÔNG tính vào chi phí lệnh mà là khoản trừ lương lái xe. Anh chốt: **gắn thẳng vào lệnh FCL v2** (không tách module/list riêng), trạm phụ CHỌN từ danh mục `TollStation` có sẵn, điều vận nhập (cùng quyền sửa lệnh), khóa theo `postB1Locked` như các phần khác của lệnh.
- **SQL** `NewAPI/Migration_DispatchOrderFCL_EtcPenalty_20260908.sql` — bảng MỚI `DispatchOrderFCLEtcPenalty` (Id/RefNo/TollStationId/TollStationName/PassedDate/Cost/Note/IsDeductedSalary/DeductedDate/CreatedBy/CreatedDate/UpdatedBy/UpdatedDate/Deleted) + 4 SP mới (`SP_DispatchOrderFCLEtcPenalty_GetByRefNo/_Create/_Update/_Delete`). 100% ADDITIVE — KHÔNG đụng `DispatchOrderFCLEtc`/`SP_DispatchOrderFcl_GetByRefNoWithTO`/`CreateWithTO`/`UpdateWithTO` (cùng pattern ExtraSegments — BE tự gọi thêm 1 SP riêng trong `GetByRefNoWithTOAsync`).
- **BE**: model `DispatchOrderFCLEtcPenalty` mới; `DispatchOrderFCL.ListEtcPenalty` (field mới); `IDispatchOrderFCL` +3 method (`AddEtcPenaltyAsync/UpdateEtcPenaltyAsync/DeleteEtcPenaltyAsync`); `DispatchOrderFCLRepository` impl 3 method + gọi `SP_DispatchOrderFCLEtcPenalty_GetByRefNo` trong `GetByRefNoWithTOAsync` (gate `IsLegacy != true`, cùng chỗ với ExtraSegments); `DispatchOrderFCLController` +3 endpoint `AddEtcPenalty/UpdateEtcPenalty/DeleteEtcPenalty` (`[ClaimRequirement(FunctionCode.DISPATCHORDER, ActionCode.UPDATE)]`, cùng pattern `AddExtraSegment`).
- **FE**: model `DispatchOrderFclEtcPenalty` mới (`models/fcl/`) + field `listEtcPenalty` trên `DispatchOrderFcl`; `DispatchOrderFclService` +3 method (`addEtcPenalty/updateEtcPenalty/deleteEtcPenalty`); `modal-dispatch-order-fcl-v2` — bảng mới "Vé ETC ngoài kế hoạch (trừ lái xe)" ngay dưới bảng ETC hiện có (section "Thông tin cung đường"), KHÔNG cộng vào `totalEtcCost`/Tổng ETC; mỗi dòng auto-save riêng (nút Lưu gọi Add/Update theo có/không `id`, nút Xóa gọi Delete có confirm nếu đã lưu); khóa toàn bộ input theo `postB1Locked` (ẩn nút Thêm/Lưu/Xóa khi đã Duyệt B1); chọn trạm từ `listTollStation` đã load sẵn (tái dùng, không gọi API mới).
- **Verify**: BE `dotnet build` — 0 Error (chỉ warning CA2200 có sẵn từ trước, không liên quan). FE `npx tsc --noEmit` — không phát sinh lỗi mới (6 lỗi còn lại đều pre-existing: spec files + `driver-fuel-closing:135`).
- **CÒN**: (1) anh review + chạy `Migration_DispatchOrderFCL_EtcPenalty_20260908.sql` (login delta.erp); (2) tắt API → `dotnet build`/publish + `ng build` deploy FE; (3) test E2E — mở 1 lệnh FCL v2 đã có refNo → "Thêm dòng" → chọn trạm + nhập ngày giờ/số tiền/ghi chú → Lưu → tải lại lệnh còn đúng dòng; Tổng ETC KHÔNG đổi; sau Duyệt B1 (status≥5) → không còn nút Thêm/Lưu/Xóa, input disabled; tick "Đã trừ lương" + Lưu → `DeductedDate` tự set.

## SP_Report01 (BC01) — bản tối ưu SP_Report01_V2 song song + endpoint BC01 cho site nháp — SQL + BE, CHỜ anh chạy SQL + deploy — 2026-09-07
Báo cáo doanh thu/chi phí theo lô (`ReportsController.GetBC01` → `SP_Report01`) chậm ~12s. Dựng `SP_Report01_V2` **song song** (KHÔNG đụng `SP_Report01`) để A/B test — kết quả PHẢI identical, chỉ nhanh hơn. Đo thực tế **12s → 6s**.
- **`NewAPI/Migration_Report01_V2_20260907.sql`** — tạo `dbo.SP_Report01_V2` + 2 INDEX ADDITIVE (`IX_Workflow_ShipmentId` ON `Workflow(ShipmentId) INCLUDE(Id)`, `IX_DispatchOrderDetailed_WorkflowId` ON `DispatchOrderDetailed(WorkflowId) INCLUDE(RefNo)`, đều `IF NOT EXISTS`; ⚠ build khóa ghi ~2-3s → chạy giờ thấp tải). ~11 tối ưu "chứng minh được tương đương": `@tbC` table-var → `#tbC` temp + PK; bỏ khối copy biến `@_KeyWord/...`; `#tmpDebit` `SELECT *`→3 cột + lọc `#sids` + clustered index; thêm `#sids` (DISTINCT ShipmentId, PK) thay mọi `IN (SELECT ShipmentId FROM #tb1)`; final `CAST(NVARCHAR(MAX))`→`(50)`/`(10)` (đã verify độ rộng nguồn không tràn); `SET NOCOUNT ON`; clustered index `#tb1(ShipmentId)`; **bỏ `WITH RECOMPILE` proc-level** (~4s compile/lần), chỉ `#tb1` giữ `OPTION(RECOMPILE)`. **Phase B**: `#tb6` bỏ view `V_DispatchOrderFCLFee` (lồng 3 tầng + 9 `FOR XML PATH`/dòng × 34k lệnh) → `#fclCnt/#fclDet/#fclFee` join base table, tái tạo nguyên văn 4 nhánh + ÷TotalDetails + CAST FLOAT (16→0.5 CPU-giây). **Phase A**: `#tb3` bỏ view `V_DispatchOrderFee` (5 nhánh UNION ALL KHÔNG có WHERE) → `#doSpine`(#sids→Workflow→DispatchOrderDetailed→DispatchOrder) + `#doFee` (5 nhánh, chia `Cost/TotalWorkflows` per-nhánh y như view) (15→~1 CPU-giây).
- **Giữ nguyên có chủ đích** (để output identical): INNER JOIN `GroupFee` dù dead ở vài #tbN (9/545 Fee có GroupFeeId NULL → join đang lọc); RIGHT JOIN Payments/PaymentsCBT; LEFT JOIN OtherCategories thiếu `Type='SHIPMENT_T02'` ở #tb2..#tb6.
- **`NewAPI/Report01_V2_ABTest_20260907.sql`** — KHÔNG deploy, chạy tay SSMS: `INSERT #a EXEC SP_Report01` / `#b EXEC SP_Report01_V2` (schema 50 cột trích từ result set thật), EXCEPT 2 chiều (kỳ vọng 0), tổng SoTien/VAT/TongTien theo nhóm `N`, mục 6 tách lệch theo `N`. **Anh đã chạy 2 bộ (2026-09-07): `bac.vu` (auth 0 → nhánh tất-cả-KH) + `anh.phuong` (`c4c53d5d-...`, auth 3 → nhánh `PermissionCS`) → EXCEPT = 0 hai chiều, tổng khớp từng số lẻ (28850 dòng).** Còn nên chạy nốt bộ @CustomerId + @KeyWord.
- **BC01 cho SITE NHÁP** (anh chốt: KHÔNG làm SP riêng): `SP_Report01_V2` thêm `@UserId UNIQUEIDENTIFIER = NULL`; khối `#tbC`: `IF @UserId IS NULL SET @quyen = 0` (quyền lớn nhất → nhánh tất-cả-KH, đã A/B test); ELSE giữ nguyên. BE thường vẫn truyền `@UserId` → KHÔNG đổi hành vi. (File `Migration_Report01_Draft_*.sql` từng tạo rồi XÓA.)
- **BE** [ReportsController.cs](../../../NewAPI/API/Controllers/Reports/ReportsController.cs) — action mới `GetBC01Draft` `[HttpPost("get-report-01-draft")]`, KHÔNG `[ClaimRequirement]`, **khóa cứng `aud=draft`** inline (`User.FindAll("aud").Any(c => c.Value == _configuration["Draft:Audience"] ?? "draft")` → không thì `Forbid()`), gọi `SP_Report01_V2` **KHÔNG Add `@UserId`** → tất cả KH; params `@BranchId/@TuNgay/@DenNgay/@CustomerId/@KeyWord/@Type/@DateType` từ body giống `GetBC01`; trả `PagedResult<ReportViewModel>`. Constructor thêm `IConfiguration`. `appsettings.json` `Draft:ReadAllowlist` thêm `/api/reports/get-report-01-draft`. ⚠ `DraftAudienceGuardFilter` HIỆN ĐANG TẮT (Program.cs ~dòng 86, tạm gỡ 2026-06-02) → bảo vệ thực tế lúc này CHỈ là check `aud=draft` inline.
- **Verify**: `dotnet build` (NewAPI) → 0 Error.
- **CÒN**: (1) anh chạy `Migration_Report01_V2_20260907.sql`; (2) deploy API (tắt API → build → publish); (3) khi anh muốn — đổi BE chính `GetBC01` `"SP_Report01"`→`"SP_Report01_V2"`; (4) anh hướng dẫn làm phía DraftAPI (process riêng) gọi `get-report-01-draft`.

## FCL — Tách rõ: lệnh CŨ = luôn thầu phụ / lệnh MỚI (v2) = luôn xe nhà — FE-only, tsc 0 lỗi mới, CHỜ ng build + deploy — 2026-09-03
Anh chốt: bỏ hẳn lựa chọn "Xe thuê ngoài" trong 2 modal FCL; phân loại cứng theo màn. Khi bấm nút lập lệnh ở `shipping-task-opman`: nút mở modal FCL cũ = lập lệnh **thầu phụ**, nút mở modal FCL v2 = lập lệnh **xe nhà**. "Bên trong cũng đổi theo" — modal cũ mở thẳng ở dạng thầu phụ, modal v2 ở dạng xe nhà. **Thuần FE, KHÔNG đụng BE/SP** (BE vẫn đọc `entity.isSubcontractors` như cũ). Xem/sửa lệnh CŨ (mọi loại) giữ nguyên — checkbox chỉ ẩn khi `flagNew`.
- **`shipping-task-opman.component.html`**: nút `createDispatchOrder()` (modal FCL cũ) "Lập lệnh vận chuyển" → **"Lập lệnh thầu phụ"**; nút `createTransportOrder()` (modal v2) "Lập lệnh (Location)" → **"Lập lệnh xe nhà"**. Chỉ đổi text.
- **`modal-dispatch-order-fcl` (FCL cũ)**: `.html` — bọc `*ngIf="!flagNew"` quanh `<div>` chứa checkbox "Xe thuê ngoài" (ẩn khi tạo mới; xem/sửa lệnh cũ vẫn thấy, disabled như trước). `.ts add()` — entity khởi tạo `isSubcontractors: true` + set `this.isLocal=false; this.isSubcontractors=true;` sau khi build entity. `.ts saveChange()` — guard `if (this.flagNew && !this.entity.shippingUnitId)` → báo lỗi "Vui lòng chọn Nhà cung cấp (nhà thầu phụ)!" + return (bổ sung cho `[required]` đã có sẵn trên ng-select shippingUnitId).
- **`modal-dispatch-order-fcl-v2` (FCL mới)**: `.html` — bọc `*ngIf="!flagNew"` quanh CẢ `form-group` chứa checkbox "Xe thuê ngoài" (ẩn nguyên hàng khi tạo mới). `.ts add()` — entity khởi tạo `isSubcontractors: false` (tường minh). **Layout**: gỡ "Loại Cont" khỏi hàng checkbox (tránh lơ lửng giữa hàng khi ẩn checkbox), chuyển xuống **cùng hàng "Tổng dầu lệnh"**: `Tổng dầu lệnh (col-2) | giá trị (col-2) | ghi chú "= Tổng dầu định mức + Dầu máy phát" (col-3, chữ mờ) | Loại Cont (col-2) | ng-select (col-3)`.
- **Verify**: `npx tsc --noEmit` — không phát sinh lỗi mới (6 lỗi còn lại đều pre-existing: spec files + `driver-fuel-closing:135`).
- **CHỜ**: `ng build` + deploy FE. Test — từ `shipping-task-opman`: 2 nút đúng tên mới; "Lập lệnh thầu phụ" mở modal cũ KHÔNG có checkbox, bắt chọn Nhà cung cấp mới lưu được; "Lập lệnh xe nhà" mở modal v2 KHÔNG có checkbox, mặc định xe nhà, hàng Loại Cont hiển thị đúng bên phải hàng Tổng dầu lệnh. Mở lại 1 lệnh FCL cũ dạng xe nhà (isSub=0) từ list cũ → vẫn xem/sửa bình thường.

## FCL v2 — Thêm trạm thu phí THỦ CÔNG theo cung (Vietmap thiếu trạm) — FE-only, ✅ ĐÃ ng build + DEPLOY (2026-09-03) — 2026-08-24
...
- **Verify**: `npx tsc --noEmit` không phát sinh lỗi mới.
- **✅ ĐÃ ng build + DEPLOY (2026-09-03)**. **CÒN**: test E2E — thêm trạm tay vào 1 cung của lệnh MỚI (chưa lưu) → lên đúng bảng ETC, màu tím; Lưu → mở lại (`edit()`) → thêm tiếp 1 trạm tay khác vào CÙNG cung đó → không bị nhân đôi các trạm cũ; sửa tay số tiền 1 trạm (auto hoặc tay) → bảng ETC tổng cập nhật ngay; xóa 1 trạm ở khối theo cung → dòng tương ứng ở bảng ETC tổng cũng mất; bấm "Tính lại lộ trình" cho cung có trạm tay → trạm tay bị xóa theo (đúng ý đã chốt).

## FCL v2 — Khóa sửa lệnh: quay về "sửa tự do tới trước Duyệt B1" — FE+BE, ✅ ĐÃ DEPLOY (2026-09-03) — 2026-08-24 — chi tiết memory `project_fcl_route_lock_policy`
...
- **Verify**: BE `dotnet build -t:Compile` 0 lỗi; FE `npx tsc --noEmit` không phát sinh lỗi mới.
- **✅ Tắt API → `dotnet build`/publish (BE) + `ng build` deploy FE — ĐÃ DEPLOY (2026-09-03)**. **CÒN**: test E2E — sửa xe/lái xe/route ở status 1-2 → Lưu → tải lại còn đúng giá trị mới; từ status 3 trở đi (đã Duyệt B1) → mọi thứ khóa lại như cũ (không bị BE âm thầm ghi đè lẫn không sửa được ở FE).

## Đọc số container AI (Gemini) — engine + component dùng chung, ĐÃ BUILD SẠCH, CHƯA nhúng vào màn nào — 2026-08-21
Anh muốn 1 API độc lập đọc số container từ ảnh (giống đọc hóa đơn), tái sử dụng được ở nhiều chỗ. Khác hóa đơn: không cần lưu DB (đọc xong dùng ngay) → thiết kế gọn hơn nhiều, không temp-folder/S3/retry.
- **BE** [GeminiAIRepository.cs](../../../NewAPI/API/Repositories/CustomerCommunicate/GoogleServices/GeminiAIRepository.cs) — thêm `ExtractContainerNumber(List<IFormFile> files)`: nhận NHIỀU ảnh của CÙNG 1 container (góc/khoảng cách khác nhau, tối đa 6) trong 1 lần gọi Gemini, prompt riêng đọc số container ISO 6346 (4 chữ + 7 số), trả 1 object JSON (khác hóa đơn — không phải mảng). Tự tính lại check-digit ISO 6346 (bảng giá trị chữ cái + trọng số 2^vị trí, bỏ qua bội số 11) và so khớp với ký tự cuối AI đọc được — không phụ thuộc AI đúng 100%. Endpoint mới `POST /api/geminiAI/extract-container` ([GeminiAIController.cs](../../../NewAPI/API/Controllers/CustomerCommunicate/GoogleServices/GeminiAIController.cs)). Model mới `ContainerExtractionResult` trong `DocumentAIModels.cs`. Dùng chung `GoogleServices:Gemini:ApiKey`/`ModelId` đã cấu hình sẵn — không cần setup thêm.
- **FE** component dùng chung `modal-doc-container` (`src/app/shared/components/transports/modal-doc-container/`) — theo khuôn picker (`SelectItem`/`CloseModal`): chọn/chụp tối đa 6 ảnh → xem trước thumbnail → "Đọc số container" → hiện số cont (sửa tay được) + badge khớp/lệch check-digit + ghi chú AI nếu không chắc → "Dùng số này" emit về form cha. Chưa nhúng vào modal/màn nào — thiết kế để nhúng lại được ở bất kỳ đâu cần ô "Số container" (field FE convention `contNo`, khớp `ShipmentContSeal`/`WorkflowContSeals`).
- **Verify**: `dotnet build -t:Compile` (NewAPI) → 0 Error (1012 warning CA2200 có sẵn từ trước). `npx tsc --noEmit` → không phát sinh lỗi mới ở file mới (6 lỗi hiện có đều pre-existing, không liên quan).
- **✅ ĐÃ NHÚNG PILOT (2026-08-21, anh chỉ định)**: nút "Đọc số cont (AI)" ở toolbar list `dispatch-order-fcl-new`, `*ngIf="adminPermission"` — CHỈ Admin thấy (đúng khuôn nút "Phân quyền chốt" cạnh bên: `dof-btn--admin`). Chưa gắn vào field nào của lệnh FCL (model không có cột Số container) — bấm xong chỉ hiện toast "Đã đọc số container: …" để test độ chính xác thực tế. `ng build` 0 lỗi mới.
- **Fix prompt sau test thật đầu tiên (2026-08-22)**: anh test ảnh container thật (số thật `IAAU2704620`, tự verify bằng thuật toán check-digit em code — 3663 mod 11 = 0, khớp) → AI đọc thiếu chữ "I" đầu (chữ I in tách xa hẳn 3 chữ AAU, dễ bị model nhầm là logo/ký hiệu trang trí). Không "train" lại được (Gemini API có sẵn, không finetune) — đã sửa **prompt** trong `GeminiAIRepository.cs`: nhấn mạnh containerNo LUÔN đúng 11 ký tự, cảnh báo rõ lỗi "ký tự đầu bị in tách xa dễ bỏ sót", bắt tự đếm lại đủ 11 ký tự trước khi trả lời. `dotnet build` → 0 Error.
- **Fix lần 2 (2026-08-22, vẫn thiếu chữ I sau fix lần 1)**: đổi 2 việc cùng lúc — (a) `mediaResolution` `MEDIUM`→`HIGH` (ảnh anh chụp là toàn cảnh xe+cont từ xa, chữ nhỏ dễ mất chi tiết ở mức MEDIUM, giống bài học invoice reader); (b) viết lại prompt bắt AI trả thêm `charCount` (tự đếm lại độ dài containerNo, phải =11) + `checkedSeparateLetter` (tự xác nhận đã tìm chữ cái đứng tách riêng trước cụm 3 chữ chưa) — ép tự-kiểm qua field JSON thay vì chỉ dặn bằng lời; bỏ `thinkingConfig.thinkingBudget=0` (cho model suy nghĩ mặc định) để hỗ trợ bước tự-đối-chiếu này. `dotnet build` → 0 Error.
- **✅ ĐÃ REDEPLOY + TEST OK (anh xác nhận 2026-08-22)**: HIGH đọc đúng đủ 11 ký tự `IAAU2704620`, không còn thiếu chữ đầu. Không còn việc tồn đọng về độ chính xác.
- **Băn khoăn token/chi phí ở mức HIGH**: chưa có số đo thật như hồi test hóa đơn (LOW≈66/MEDIUM≈258 token/ảnh, đo hồi đó). Đã giải thích: modal đã tự hiện đúng số token thật ở footer sau mỗi lần đọc (`usageMetadata` của chính response) — không cần đoán, anh xem trực tiếp; và đây là API dùng theo yêu cầu (không batch hàng loạt như hóa đơn) nên chi phí tuyệt đối rất nhỏ dù HIGH tốn hơn MEDIUM vài lần. Nếu sau này thấy token cao bất thường → cân nhắc giới hạn còn 3-4 ảnh/lần thay vì 6.

## FCL — Phân quyền CHỐT LỆNH theo KH: 3 fix UI trang quản lý (màu nút + dropdown tài khoản trắng + list KH tràn cột) — FE-only, ĐÃ DEPLOY + TEST OK (anh xác nhận 2026-08-21)
Anh xem trước bản dựng modal `modal-fcl-closing-scope` (chưa deploy, chỉ soạn xong SQL/BE/FE tuần trước — xem section trên), phát hiện 2 lỗi UI ngay khi review:
1. **Nút "Phân quyền chốt"** ở toolbar `dispatch-order-fcl-new` chỉ dùng class `dof-btn` trơn (không modifier màu) — class gốc chỉ set `color:#fff` không có `background` ⇒ nút gần như chìm trên toolbar nền trắng, không nổi bật đúng mức "thao tác nhạy cảm chỉ Admin". Thêm modifier mới `&--admin{background:#7c3aed}` (tím, tách biệt hẳn khỏi primary/success/danger/warning/chot đang dùng) vào [dispatch-order-fcl-new.component.scss](../../src/app/main/transports/dispatch-order-fcl-new/dispatch-order-fcl-new.component.scss) + gắn class vào nút.
2. **Dropdown "Tài khoản" trong form gán quyền trắng tinh** (có item nhưng không hiện chữ): bind `bindLabel="employeeFullName"` trong khi nguồn `listUser` lấy từ `_userService.getAll()` → `SP_sys_User_GetAll` — đã đọc DB (SELECT only) xác nhận SP này chỉ trả `Id,UserName,Email,PhoneNumber,FullName,Address`, **không có cột `EmployeeFullName`**; thử luôn `Users.FullName` cũng rỗng 100% ở data thật (10/10 dòng mẫu). Đổi `bindLabel` sang `userName` (luôn có dữ liệu) — đúng khuôn `modal-permission-payment` đã né sẵn bug này cho cùng nguồn `User[]`.
3. **List "Phạm vi" tràn ngang bóp cột "Tài khoản"**: badge tên KH nối ngang (`*ngFor` inline) khiến hàng dài đẩy cột Tài khoản co lại, tên user 2-3 chữ bị xuống dòng xấu. Đổi mỗi KH xuống 1 dòng riêng (`<div>` thay vì `<span>` liền kề) + cố định `table-layout:fixed` với cột Tài khoản `width:160px`.
- File sửa: [modal-fcl-closing-scope.component.html](../../src/app/shared/components/systems/modal-fcl-closing-scope/modal-fcl-closing-scope.component.html), [dispatch-order-fcl-new.component.html](../../src/app/main/transports/dispatch-order-fcl-new/dispatch-order-fcl-new.component.html), [dispatch-order-fcl-new.component.scss](../../src/app/main/transports/dispatch-order-fcl-new/dispatch-order-fcl-new.component.scss).
- **Verify**: `npx tsc --noEmit` — không phát sinh lỗi mới (các lỗi hiện có đều ở spec file khác, có sẵn từ trước).
- **✅ ĐÃ deploy chung nhịp với SQL/BE của tính năng** (xem section dưới — anh xác nhận 2026-08-21).

## FCL — Phân quyền CHỐT LỆNH theo khách hàng — ✅ ĐÃ DEPLOY + TEST OK (anh xác nhận 2026-08-21) — 2026-08-20
Anh muốn đổi "Chốt lệnh" FCL (status 5→6) từ 1 quyền chung `FCL_CLOSING` (ai có là chốt được mọi lệnh) sang định tuyến theo KH: lệnh 1 KH → chỉ người được gán riêng KH đó chốt được; lệnh nhiều KH → route về nhóm "Chốt tất cả" (nhóm này luôn chốt được mọi lệnh, kể cả lệnh 1 KH).
- **Thiết kế đã thảo luận + anh chốt từng bước** (đúng nguyên tắc "chốt ý tưởng trước, đừng nhảy SQL"): (1) KH của lệnh = tập CustomerId trên các điểm nhận/giao (`Tbl_TransportOrder_Segments` → `CustomerLocations`, không cần thêm cột); (2) `FCL_CLOSING` vẫn là điều kiện NỀN bắt buộc, bảng mới CỘNG THÊM lớp định tuyến KH; (3) "Chốt tất cả" luôn qua được kể cả lệnh 1 KH; (4) lệnh 1 KH chưa ai gán riêng → tự rơi về nhóm "Chốt tất cả" (không cần logic riêng); (5) áp dụng ĐỒNG NHẤT ở modal + list từng dòng + bulk "Chốt nhiều", không có ngoại lệ.
- **SQL** [Migration_FCL_ClosingScope_ByCustomer_20260820.sql](../../../NewAPI/Migration_FCL_ClosingScope_ByCustomer_20260820.sql):
  1. `Tbl_FclClosingScope` (UserId, CustomerId NULL=Tất cả) + 2 unique filtered index.
  2. `FN_FclOrderCustomers(@RefNo)` — tập KH của 1 lệnh (iTVF, JOIN segments→CustomerLocations).
  3. `FN_FclScopeOk(@UserId, @RefNo)` — 1 nếu user được phép chốt lệnh đó theo phạm vi KH.
  4. 3 SP CRUD quản lý gán quyền: `SP_FclClosingScope_GetAll/_SaveForUser/_DeleteForUser`.
  5. `SP_DispatchOrderFCL_GetAll` — thêm `@HasClosingClaim`, `@IsAdmin`, dùng lại `@UserId` (param có sẵn, trước không dùng) → cột `CanCloseFcl` (cả 2 nhánh có/không keyword).
  6. `SP_DispatchOrderFcl_GetByRefNoWithTO` — thêm `@UserId`/`@HasClosingClaim`/`@IsAdmin` → `CanCloseFcl` ở result set 1.
  7. `SP_DispatchOrderFCL_ChangeStatus` — thêm chặn THẬT theo phạm vi KH cho ActionType 4 (Chốt lệnh)/7 (Từ chối chốt), Admin bypass qua `@IsAdmin`.
  Chỉ áp dụng lệnh MỚI (IsLegacy=0) — lệnh legacy không có `Tbl_TransportOrders` tương ứng nên `CanCloseFcl` luôn 0, FE legacy vốn không đọc cột này nên không ảnh hưởng (đúng nguyên tắc không đụng list/modal FCL legacy).
- **BE**: `IDispatchOrderFCL`/`DispatchOrderFCLRepository` — `GetAll`/`GetByRefNoWithTOAsync`/`ChangeStatusAsync` thêm param `userId/hasClosingClaim/isAdmin` (optional, default false — không phá caller cũ). `DispatchOrderFCLViewModel` +`CanCloseFcl`. `DispatchOrderFCLController` (`GetPaging`/`GetByRefNoWithTO`/`ChangeStatus`) tính `User.HasPermission("FCL_CLOSING")` + `User.IsInRole("Admin")` rồi truyền xuống. Module quản lý mới: `Models/ViewModels/Interfaces/Repositories/Controllers` `FclClosingScope` (`GetAll` ai có quyền FCL đều xem; `SaveForUser`/`DeleteForUser` CHỈ Admin — gán quyền chốt lệnh là thao tác nhạy cảm, chưa có FunctionCode riêng).
- **FE**: model `DispatchOrderFcl` +`canCloseFcl`. `modal-dispatch-order-fcl-v2` (nút CHỐT LỆNH + Từ chối CHỐT) và `dispatch-order-fcl-new` (nút chốt từng dòng + `checkClosing()` cho bulk "Chốt lệnh nhiều") đổi từ `closing_permission` tĩnh sang `item.canCloseFcl`/`entity.canCloseFcl` theo từng lệnh — nhất quán cả 3 nơi, không ngoại lệ.
- **Trang quản lý gán quyền** (mới, hoàn thiện): modal `modal-fcl-closing-scope` (mirror pattern `modal-permission-payment`) — bảng danh sách gán hiện tại (gom theo user: "Chốt tất cả" hoặc chip tên từng KH) + form Sửa/Thêm (chọn tài khoản, radio Tất cả/Theo KH, `ng-select multiple` chọn KH) + nút gỡ. Mở qua nút "Phân quyền chốt" mới ở toolbar `dispatch-order-fcl-new` (chỉ hiện với `adminPermission`, khớp Save/Delete chỉ Admin ở BE). Service `FclClosingScopeService` (getAll/saveForUser/deleteForUser).
- **Verify**: `dotnet build -t:Compile` → 0 Error. `npx tsc --noEmit` → không lỗi mới (cả BE-adjacent lẫn UI mới), lỗi hiện có đều thuộc file khác không liên quan, có sẵn từ trước.
- **✅ ĐÃ chạy SQL (login delta.erp) → tắt API build/publish → `ng build` FE → TEST OK** (anh xác nhận 2026-08-21): gán quyền "Chốt tất cả"/"Chốt theo KH" đúng, nút CHỐT LỆNH hiện/ẩn đúng ở modal chi tiết + list + bulk, chặn đúng user ngoài phạm vi gọi thẳng API. Không còn việc tồn đọng.

## Fix double dữ liệu khi Duyệt nháp ERP (race condition ở AddFromDraft) — ✅ ĐÃ DEPLOY + TEST OK (anh xác nhận 2026-08-21) — 2026-08-19
Anh phản ánh: duyệt nháp qua ERP (site nháp → job/phiếu thật) đôi khi bị double dữ liệu, đặc biệt lúc "Duyệt nhiều" (bulk).
- **Chẩn đoán**: `AddFromDraft` ở cả 4 controller (`ShipmentController`, `PaymentsController`, `DebitNoteController`, `ShippingTaskController` — clone cùng khuôn, xem [[project_draft_promote_payment_debit]]) làm 3 bước KHÔNG nguyên tử: (1) `SP_DraftEntries_GetForPromote` SELECT đọc Status, (2) nếu `Draft` → gọi SP tạo bản ghi thật bên `dbo.`, (3) `SP_DraftEntries_MarkPromoted` UPDATE `Status='Promoted' WHERE Status='Draft'`. Giữa bước 1-3 không có khóa nào → 2 request cùng nháp gần như đồng thời (2 tab/2 người, hoặc trúng giữa lúc "Duyệt nhiều" chạy tuần tự nhiều giây trên hàng đợi nháp DÙNG CHUNG) đều đọc thấy `Draft` ở bước 1 → CẢ HAI đều tạo bản ghi thật → double ERP. Bước 3 chỉ chặn ghi trùng vào `draft.DraftEntries`, không chặn tạo trùng bên `dbo.`, và code cũ còn bỏ qua luôn `RowsAffected` của MarkPromoted (fire-and-forget trong try/catch nuốt lỗi).
- Đã kiểm tra "Duyệt nhiều" (3 màn: `shipment-normal`/Lô hàng, `job-canon`/Canon, `shipping-task-cs`/ShippingTask CS) dùng `concatMap` (tuần tự, có khóa nút `bulkBusy`) → **an toàn trong CÙNG 1 lần bấm của 1 người**; lỗ hổng chỉ lộ khi có request THỨ 2 từ nguồn khác (tab khác/người khác) chen vào giữa batch.
- **Quyết định (anh Cường chốt 2026-08-19)**: đổi guard "check rồi mới act" sang **claim nguyên tử bằng chính UPDATE có WHERE** (SQL Server tự khóa row lúc UPDATE), qua trạng thái trung gian `Promoting`.
- **SQL** [Migration_DraftSite_PromoteClaim_20260819.sql](../../../NewAPI/Migration_DraftSite_PromoteClaim_20260819.sql) — schema `draft`, không đụng `dbo.`:
  1. `draft.SP_DraftEntries_ClaimPromote` (SP MỚI) — `UPDATE Status 'Draft'→'Promoting' WHERE Status='Draft'`; `@RowsAffected=1`=thắng claim (được tạo bản ghi thật), `=0`=đã có người khác xử lý/đã xong.
  2. `draft.SP_DraftEntries_ReleaseClaim` (SP MỚI) — trả `Status` về `Draft` khi bước tạo bản ghi thật LỖI sau khi đã thắng claim (tránh nháp kẹt vĩnh viễn ở `Promoting`).
  3. `draft.SP_DraftEntries_MarkPromoted` (DROP+CREATE, sửa 1 điều kiện) — `WHERE Status='Draft'` → `WHERE Status='Promoting'`, phần còn lại giữ nguyên y hệt.
  KHÔNG đụng `SP_DraftEntries_GetForPromote` (vẫn dùng để trả thông tin "đã duyệt trước đó") và mọi SP tạo bản ghi thật bên `dbo.`.
- **BE**: `IDraft`/`DraftRepository` thêm `ClaimPromote(id, reviewedBy)` + `ReleaseClaim(id)`. Cả 4 controller (`ShipmentController.AddFromDraft`, `PaymentsController.AddFromDraft`, `DebitNoteController.AddFromDraft`, `ShippingTaskController.AddFromDraft`) đổi luồng: validate → `ClaimPromote` (claim=0 → trả "đang/đã được duyệt bởi thao tác khác", KHÔNG tạo bản ghi thật) → `try { tạo bản ghi thật thật (tạo lỗi → ReleaseClaim + trả 500) → MarkPromoted → trả 201 } catch { ReleaseClaim best-effort rồi throw ra ngoài (outer catch cũ format lỗi 500 như trước) }`.
- **Không đụng**: FE "Duyệt nhiều" (`concatMap`/`bulkBusy` đã đúng, không cần sửa gì — cả 3 màn tự động hưởng lợi vì cùng gọi `addFromDraft`), `SP_DraftEntries_GetForPromote`, mọi SP tạo bản ghi thật (`SP_Shipment_Create`/`SP_Payments_Create`/...), FE `saveDraft`/`viewDraft`/`updateDraft`.
- **Verify**: `dotnet build -t:Compile` (NewAPI) → **0 Error** (1012 warning đều CA2200 có sẵn từ trước, không liên quan).
- **✅ ĐÃ chạy SQL + tắt API build/publish** (anh xác nhận 2026-08-20). **✅ TEST OK (anh xác nhận 2026-08-21)**: 2 tab/2 tài khoản duyệt trùng 1 nháp → chỉ 1 bản ghi thật; "Duyệt nhiều" bình thường không regression; test case tạo lỗi giữa chừng → nháp trả về `Draft` (không kẹt `Promoting`). Không còn việc tồn đọng.
- **Rollback (đề phòng lỗi)**: SQL — [Migration_DraftSite_PromoteClaim_ROLLBACK_20260819.sql](../../../NewAPI/Migration_DraftSite_PromoteClaim_ROLLBACK_20260819.sql) khôi phục đúng body cũ của `SP_DraftEntries_MarkPromoted` (`WHERE Status='Draft'`); 2 SP mới (`ClaimPromote`/`ReleaseClaim`) để nguyên, vô hại nếu BE cũ không gọi tới. BE — repo `NewAPI` là git, đang ở commit `082b6fa` (trước khi sửa 6 file lần này), CHƯA commit gì mới → rollback BE chỉ cần `git checkout -- API/Controllers/AdvanceAndPayments/PaymentsController.cs API/Controllers/Shipments/DebitNoteController.cs API/Controllers/Shipments/ShipmentController.cs API/Controllers/Transports/ShippingTaskController.cs API/Interfaces/IDraft.cs API/Repositories/DraftRepository.cs` rồi build/deploy lại — không cần file backup riêng. ⚠ Nếu đã deploy bản MỚI 1 thời gian rồi mới rollback, phải chạy SQL rollback SAU/CÙNG lúc rollback BE (không để BE mới chạy chung SP MarkPromoted bản cũ — nháp sẽ kẹt vĩnh viễn ở `Promoting`); file rollback có sẵn dòng UPDATE (comment sẵn) để giải phóng nháp lỡ bị kẹt.
- **Đã commit riêng** (tách khỏi phần Chốt dầu tháng): NewAPI `97c2e70`, web-app-update `14e4af4`.
- **🚨 SỰ CỐ 2026-08-20 (sau khi chạy SQL) — bỏ sót `WorkflowController.PromoteFromDraft`**: controller này duyệt Job (`draft.DraftType='Job'`) dùng tên method KHÁC (`PromoteFromDraft` không phải `AddFromDraft`) nên lọt qua lúc rà "4 controller" hôm 2026-08-19 — vẫn theo luồng cũ, không gọi `ClaimPromote`. Sau khi SQL đổi `MarkPromoted` yêu cầu `Status='Promoting'`, luồng cũ của Workflow luôn ghi ngược thất bại (0 dòng) dù Job thật đã tạo — nháp kẹt `Draft` vĩnh viễn → không biến mất khỏi list → anh bấm lại nhiều lần → **tạo trùng thật ~167 dòng `dbo.Workflow` (IsMainJob=1) trên 25 JobId** (riêng `SSG26081203000001` trùng 64 lần), tất cả cùng 1 người dùng, phát hiện qua SELECT-only cross-check `draft.DraftEntries` (Status=Draft, DraftType=Job) với `dbo.Workflow` theo JobId. **Đã sửa** `WorkflowController.PromoteFromDraft` theo đúng khuôn Claim/Release như 4 controller kia — build 0 lỗi — commit `c752b45`. **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-21): "Duyệt nhiều" Job biến mất khỏi list đúng, không tạo trùng nữa. Dọn 167 dòng trùng: anh vẫn chọn TẠM CHƯA làm — script đã soạn sẵn [Migration_DraftSite_Job_Duplicate_Cleanup_20260820.sql](../../../NewAPI/Migration_DraftSite_Job_Duplicate_Cleanup_20260820.sql) chờ anh chạy khi tiện.

## Chốt dầu tháng — thêm nguồn "Mua dầu ngoài, tạm ứng" (ExternalOilPurchased) — ✅ ĐÃ DEPLOY + TEST OK (anh xác nhận 2026-08-21) — 2026-08-19
Anh phát hiện: `SP_DriverFuelClosing_GetCandidates` (phiếu chốt dầu tháng) đang thiếu 1 nguồn — bảng `ExternalOilPurchased` ("Mua dầu ngoài", nội bộ SP ghi rõ "MUA DAU NGOAI TIEN MAT"). Khi phiếu này Duyệt B2 (`Status=3`), `SP_ExternalOilPurchased_UpdateState` đã tự ghi NỢ DẦU cho tài xế vào `DriverFuelDebit` — về bản chất là một dạng **tạm ứng dầu** giống nguồn A (`DriverFuelApproval Type=0`) nhưng chưa từng được trừ vào phiếu chốt.
- **Khảo sát DB** (read-only, SELECT): bảng đã có sẵn cột `IsFuelClosing BIT` (100% đang =0, chưa dùng lần nào — hạ tầng chống trùng có sẵn, chỉ chưa nối dây). `VihicleId` **bắt buộc nhập** ở FE (`required`) — kiểm tra thực tế 5.923 phiếu Status=3 không phiếu nào null xe → suy luận an toàn: bảng này không có khái niệm dầu máy phát, luôn Bucket=1 (Vận hành). Không có cột `Deleted` nhưng `SP_ExternalOilPurchased_Delete` là hard-DELETE thật nên không cần lọc.
- **Anh chốt thiết kế (2026-08-19)**: (1) cửa sổ ngày lọc dùng `RefDate` (ngày hóa đơn), (2) luôn Bucket=1 Vận hành, (3) là tạm ứng dầu → gộp chung nhóm "Cấp" (A) với Source 1, dấu TRỪ giống nguồn A, chỉ lấy `Status=3` (Duyệt B2 — đã ghi nợ dầu thật).
- **SQL** [Migration_DriverFuelClosing_ExternalOilPurchased_20260819.sql](../../../NewAPI/Migration_DriverFuelClosing_ExternalOilPurchased_20260819.sql) — 3 SP (không có GRANT riêng nào, đã kiểm tra `sys.database_permissions`, dùng DROP+CREATE):
  1. `SP_DriverFuelClosing_GetCandidates` — thêm UNION Source=7 (Bucket=1) từ `ExternalOilPurchased`, `Status=3`, `RefDate` trong khoảng, chưa `IsFuelClosing`, `ReasonName` set cứng `N'Mua dầu ngoài (tạm ứng)'`.
  2. `SP_DriverFuelClosing_Create` — `@SupOper` đổi từ `SUM(...Source=1 AND Bucket=1)` → `SUM(...Source IN (1,7) AND Bucket=1)`.
  3. `SP_DriverFuelClosing_Approve` — thêm block `UPDATE ExternalOilPurchased SET IsFuelClosing=1` cho Source=7 (thiếu bước này thì phiếu Mua dầu ngoài mãi là candidate → bị đưa vào 2 phiếu chốt khác nhau).
- **FE** [modal-vehicle-fuel-closing.component.ts](../../src/app/shared/components/transports/modal-vehicle-fuel-closing/modal-vehicle-fuel-closing.component.ts): `sourceLabel()` thêm `case 7`, `recalcSummary()` `supOper` gộp `source===1 || source===7`. [modal-vehicle-fuel-closing.component.html](../../src/app/shared/components/transports/modal-vehicle-fuel-closing/modal-vehicle-fuel-closing.component.html): loop nhóm nguồn thêm `7`, cột "Lý do" hiện cho cả `s===1 || s===7`.
- **Không đụng**: Source 1-6, công thức NET tổng (vẫn `(Định mức−Cấp)+Thừa`, chỉ Cấp Vận hành nay = Source1+Source7); `SP_ExternalOilPurchased_*` (Create/Update/Delete/UpdateState/GetPaging/GetById) — không sửa nguồn dữ liệu, chỉ đọc thêm.
- **Verify**: `npx tsc --noEmit` không phát sinh lỗi mới ở 2 file FE.
- **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-21): "Chốt dầu phương tiện" cho xe có phiếu Mua dầu ngoài Status=3 trong tháng hiện đúng nhóm + số lít, Duyệt xong phiếu nguồn được đánh dấu `IsFuelClosing=1`. Không còn việc tồn đọng.

## HTTP Response Compression (gzip/brotli) cho toàn bộ API — ✅ ĐÃ DEPLOY + TEST OK (anh xác nhận 2026-08-21) — 2026-08-18
Anh hỏi: API trả list dữ liệu lớn (>1M dòng) có nén để giảm thời gian tải được không. Đã **đo thực tế** (không suy đoán): lấy 50.000 dòng thật từ bảng `Shipment` (58 cột), dựng JSON đúng hệt định dạng API trả về (camelCase), nén thử bằng gzip/brotli (Node.js zlib) — kết quả **giảm 94-97% dung lượng**, chi phí nén CPU chỉ 300-500ms/60MB (không đáng kể). Ngoại suy 1 triệu dòng: 1.22 GB → còn ~39-71 MB.
- **Quyết định**: dùng **HTTP response compression chuẩn** (`Content-Encoding: gzip/br`) — cơ chế có sẵn của HTTP, trình duyệt/client tự giải nén ở tầng mạng, **KHÔNG cần đổi 1 dòng FE nào** (không viết tay pako.js). Kiểm tra thấy repo **CHƯA bật** cái này (không có `AddResponseCompression`/`UseResponseCompression` trong `Program.cs`, `web.config` publish cũng không khai `<httpCompression>`).
- **Câu hỏi mở của anh: BE này còn phục vụ app Flutter, nén có ảnh hưởng gì không?** → Phân tích: compression là **content negotiation theo `Accept-Encoding` client tự gửi** — client không gửi/không hỗ trợ thì server tự động KHÔNG nén (fallback = hành vi hiện tại), không thể làm vỡ client nào. `dart:io HttpClient` (nền của gói `http`/`dio` khi Flutter chạy mobile/desktop) có `autoUncompress=true` mặc định → tự xin + tự giải nén **gzip** miễn phí, không cần đội mobile sửa code. Brotli thì Dart không tự xin nên server tự trả về gzip cho app đó (~94% vẫn tốt). Lưu ý duy nhất: khi nén, ASP.NET Core bỏ header `Content-Length` (chuyển chunked) — chỉ ảnh hưởng nếu app có progress bar dựa vào content-length khi tải 1 API JSON to (hiếm, và không đụng file/ảnh/PDF vì các MIME đó vốn không nằm trong danh sách được nén).
- **Code** [Program.cs](../../../NewAPI/API/Program.cs): `builder.Services.AddResponseCompression(...)` (Brotli + Gzip provider, `EnableForHttps=true`, thêm `application/json` vào MIME list) + `Configure<BrotliCompressionProviderOptions>/<GzipCompressionProviderOptions>` mức `CompressionLevel.Optimal` (khớp số đo thực tế, đủ nhanh) + `app.UseResponseCompression()` đặt **đầu tiên** trong pipeline (ngay sau `builder.Build()`, trước `UseExceptionHandler`) — đúng khuyến nghị chính thức ASP.NET Core.
- **Lưu ý bảo mật**: `EnableForHttps=true` bật nén cả trên HTTPS — về lý thuyết mở khả năng tấn công kiểu BREACH (chỉ nguy hiểm khi response vừa chứa bí mật vừa phản chiếu input do kẻ tấn công kiểm soát); API đọc dữ liệu ERP qua JWT không rơi vào case đó nên chấp nhận đánh đổi.
- **Verify**: `dotnet build -t:Compile` → **0 Error** (1012 warning đều CA2200 có sẵn từ trước, không liên quan).
- **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-21): DevTools web xác nhận header `Content-Encoding` + giảm kích thước response; app Flutter đọc dữ liệu bình thường; Innvie/EUP (`Api-Key`, không JWT) chạy bình thường. Không còn việc tồn đọng.
- **Ý tưởng KHÔNG làm (cùng phiên)**: nút "Đọc hóa đơn AI" tự điền trực tiếp vào từng dòng ở màn Chi tiết thanh toán (`payment-detail.component`) — đã lên kế hoạch xong (tái dùng endpoint có sẵn `extract-invoice`, không cần BE mới) nhưng **anh yêu cầu bỏ**, không code.

## Fix bug chốt phiếu cấp dầu IGAS chặn nhầm khi S/L thực tế = 0 — FE-only, tsc 0 lỗi mới, CHỜ ng build/deploy — 2026-08-15
Anh phản ánh: xuất phiếu IGAS nhưng lái xe không đổ → không chốt được phiếu để ghi nhận trả tiền dầu cho lái xe. Điều tra ra **cùng 1 bug lặp giống hệt ở 2 modal**:
- [modal-driver-fuel-approval.component.ts:127](../../src/app/shared/components/transports/modal-driver-fuel-approval/modal-driver-fuel-approval.component.ts) (phiếu cấp dầu chung) và [modal-fuel-summary.component.ts:102](../../src/app/shared/components/transports/modal-fuel-summary/modal-fuel-summary.component.ts) ("Cấp dầu theo lệnh" — chốt lệnh FCL): hàm `chotPhieu()` có `if (!entity.refuelingTimeIgas || entity.quantityIgas == 0) return;` — chặn IM LẶNG (không báo lỗi) khi S/L thực tế = 0, coi "chưa nhập" và "nhập 0 thật" là một. BE (`DriverFuelApprovalRepository.Approved()`) KHÔNG có chặn này — thuần bug FE.
- **Fix theo đúng ý anh** ("nếu không đổ chỉ cần nhập igasquantity=0, bắt buộc phải nhập chứ không để trống, không cần nhập thời gian đổ vẫn được chốt"): đổi guard thành chỉ chặn khi `quantityIgas` thật sự để trống (`== null` hoặc `''`), có báo lỗi rõ "Vui lòng nhập S/L thực tế (nhập 0 nếu lái xe không đổ)"; **bỏ hẳn** điều kiện bắt buộc `refuelingTimeIgas`. Giá trị 0 giờ hợp lệ để Chốt dữ liệu. Thêm dấu `*` bắt buộc ở nhãn "S/L thực tế" trong 2 file HTML tương ứng.
- ⚠ Lưu ý kỹ thuật: cột `QuantityIgas` (BE model `decimal` không nullable) mặc định 0 ngay khi xuất phiếu — nên "chưa nhập" vs "nhập 0" khó phân biệt tuyệt đối ở tầng dữ liệu, nhưng đáp ứng đúng yêu cầu (0 hợp lệ, không cần thời gian đổ).
- **Verify**: `npx tsc --noEmit` không phát sinh lỗi mới ở 2 file.
- **✅ ĐÃ ng build + DEPLOY** (anh xác nhận 2026-08-18). **CÒN**: test thật — xuất phiếu → không đổ, nhập S/L thực tế=0 → bấm Chốt dữ liệu → chốt thành công (trước đây bị chặn im lặng).

## FCL v2 — Duyệt B1 gate (chặng cuối) + khóa toàn bộ sau B1 (postB1Locked) + fix bug route mutation — FE-only, tsc 0 lỗi mới, CHỜ ng build/deploy — 2026-08-10
Modal `modal-dispatch-order-fcl-v2`. Chuỗi 3 yêu cầu liên tiếp cùng ngày:
1. **Chuyển gate "chặng cuối" từ Lưu sang Duyệt B1**: trước đây bắt chọn "Chặng cuối" mới Lưu được lệnh mới; anh yêu cầu bỏ ràng buộc ở Lưu, chuyển sang chặn nút "Duyệt B1" (`[disabled]="!lastSegmentFinal"` + tooltip, guard đầu `duyetB1()`). ⚠ Lưu ý kỹ thuật: `edit()` luôn set `lastSegmentFinal=true` khi mở lại lệnh đã có refNo (chặng cuối = phần tử cuối mảng, không lưu DB) ⇒ gate này gần như luôn pass ở luồng Duyệt B1 thực tế, chỉ thực sự chặn nếu bấm Lưu mà chưa từng chọn chặng cuối rồi thoát ra vào lại — đã báo anh.
2. **Khóa toàn bộ thông tin dầu sau Duyệt B1** bất kể xem/sửa: thêm getter `postB1Locked = flagXem || entity.status>=5` (đổi tên từ `fuelLocked` ban đầu), áp cho `oilCompensation`/`reasonOilCompensation`/`generatorRunningHours`/`generatorTemperature`/`generatorFuelNorm` + nút Lưu dầu máy phát + guard `saveGenerator()`.
3. **Tổng kết "sau B1 chỉ còn chờ CHỐT LỆNH"**: mở rộng `postB1Locked` ra bảng chi phí (`feeId` ng-select — trước đó KHÔNG hề khóa dù ở chế độ xem, đã vá; `contents`/`quantity`/`cost`/`vat`/`note`), "Tóm tắt Lệnh" + "Ghi chú". **Phát hiện bug có sẵn từ trước** (không phải do phiên này gây ra) khi rà soát: khu vực danh sách điểm route (xóa điểm/kéo-thả sắp xếp/sửa giá trạm Vietmap từng chặng/xóa trạm) hoàn toàn KHÔNG check `routeConfirmed` — route "đã khóa" (có refNo) vẫn sửa được ở FE, và Duyệt B1 sau đó sẽ lưu luôn route bị mutate ngầm. Đã vá bằng `cdkDropListDisabled`/`cdkDragDisabled`/`*ngIf`/`[disabled]` + guard đầu hàm `removeLocation()`/`onDropLocation()`/`removeStation()`.
- **Không khóa** (chủ đích): `attack()`/`attackDriver()` (đính kèm hồ sơ theo shipping-task) và `duplicate()` — hành động cấp shipping-task, không sửa dữ liệu chính lệnh FCL.
- **Verify**: `npx tsc --noEmit -p tsconfig.json` nhiều lượt, chỉ còn 6 lỗi pre-existing không liên quan.
- **✅ ĐÃ ng build + DEPLOY** (anh xác nhận 2026-08-18). **CÒN**: test thật — Lưu lệnh mới không cần chọn chặng cuối → Duyệt B1 bị chặn nếu chưa chọn chặng cuối (tooltip) → sau Duyệt B1 mọi ô dầu/phí/tóm tắt/ghi chú/route đều khóa dù mở ở chế độ sửa.

## (Concept only, CHƯA CODE) FCL — Lệnh vận tải phụ (mô hình cắt mooc LG/Pantos/Canon) — 2026-08-10
Bàn giải pháp cho nhu cầu 1 container có nhiều lái xe/xe (cắt mooc). Chốt Ý TƯỞNG: dùng CHUNG bảng `DispatchOrderFCL` + bảng liên quan cho cả lệnh chính/phụ (không tách bảng riêng), thêm field phân biệt + liên kết lệnh cha, khác biệt chủ yếu ở FE hiện đúng tập field cần. Tái dùng nguyên `SP_DispatchOrderFCL_ChangeStatus` (thao tác theo RefNo, không phân biệt chính/phụ). Chi tiết đầy đủ + các điểm CHƯA quyết ở memory `project_fcl_sub_order.md`. **Đang PAUSE theo yêu cầu anh** — không viết SQL/BE/FE (kể cả nháp) tới khi anh chủ động báo bắt đầu.

## FCL v2 — "Check EUP" đối chiếu phí cầu đường (list + modal ETC) + 2 fix hiển thị list — FE-only, ng build 0 err, ĐÃ COMMIT+PUSH main — 2026-08-07
Điều vận cần đối chiếu ETC tự nhập với dữ liệu thật từ EUP (thiết bị GPS) cho lệnh FCL v2 đã có đủ T/g bắt đầu + kết thúc. Dùng lại `EupfinController.GetTollFee` có sẵn (BE, đã commit `326306b` từ trước, không cần sửa BE lần này).
- **Phát hiện qua đọc SP thật (read-only)**: `SP_DispatchOrderFCL_GetAll` (SP nuôi list) thiếu `StartedDate/FinishedDate` trong SELECT dù model đã có sẵn 2 cột (2 cột này chỉ có ở `GetByRefNoWithTO`, SP dùng cho modal). Anh đã tự ALTER thêm vào nhánh KHÔNG-keyword; nhánh CÓ keyword còn thiếu (đã báo anh, chưa sửa — lọc theo từ khóa sẽ tạm mất nút ở list, modal không ảnh hưởng).
- **Modal mới** `modal-eup-toll-check` (`src/app/shared/components/transports/modal-eup-toll-check/`): đọc-only, `show(vehiclePlate, startedDate, finishedDate)` gọi `POST /api/Eupfin/GetTollFee` (format `moment().format('YYYY-MM-DD HH:mm:ss')`), bảng Trạm/Thời gian/Số tiền + tổng, báo lỗi qua `resultMsg` khi `resultCode≠0`. Ghi đè CSS `:host ::ng-deep .ng-busy{margin-top:0}` riêng cho modal này — global `busy.css` mặc định `margin-top:260px` (hợp cho modal to/form dài), làm icon loading trôi xuống giữa/dưới màn hình với modal ngắn kiểu này.
- **List** `dispatch-order-fcl-new`: nút "Check EUP" (`dof-btn--warning`) ở cột Tác vụ, `*ngIf="item.startedDate && item.finishedDate"`.
- **Modal** `modal-dispatch-order-fcl-v2`: thêm hiển thị "T/g bắt đầu lệnh" (trước đây không có ô nào hiển thị `entity.startedDate`) + nút "Check EUP" trong header phần ETC, cùng điều kiện.
- **2 fix CSS list `dispatch-order-fcl-new` phát hiện khi anh xem thật** (không liên quan tính năng Check EUP, lộ ra do thêm cột cạnh):
  1. `tbody td { vertical-align: top }` — trước đó `middle` khiến các ô 1 dòng (Tổng Km/Tổng dầu/Ghi chú) trôi lơ lửng giữa hàng khi cột "Điểm giao nhận hàng" xuống 2 dòng làm hàng cao lên.
  2. **Bug CSS specificity có từ trước**: `.dof-col-lg`/`.dof-col-note` (`white-space:normal` để chữ xuống dòng) **không hề có tác dụng** vì `.dof-table td{white-space:nowrap}` (class+element, specificity cao hơn 1 class) luôn thắng — địa chỉ dài ở cột "Điểm giao nhận hàng" tràn ngang đè sang cột Tổng Km thay vì xuống dòng gọn trong cột. Sửa bằng khai `td.dof-col-lg, td.dof-col-note { white-space: normal }` LỒNG trong block `.dof-table` để thắng specificity.
- **Verify**: `npx tsc --noEmit` 6 lỗi pre-existing (không liên quan) + `npx ng build` (default config) 0 lỗi mới qua nhiều lượt build lại trong phiên.
- **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-07). Còn 1 ghi chú nhỏ không chặn: bổ sung nốt nhánh CÓ keyword của `SP_DispatchOrderFCL_GetAll` (thiếu `StartedDate/FinishedDate`) khi rảnh.

## FCL legacy — modal MỚI "Lịch sử lệnh" (lý do từ chối + timeline) — BE+FE, build 0 err, ĐÃ COMMIT+PUSH cả 2 repo — 2026-08-06/07
Anh hỏi lý do từ chối ở bước chốt lệnh cuối cùng (lệnh FCL **cũ**, khác v2) ghi ở đâu để mọi người xem được. Truy ra: `SP_DispatchOrderFCL_UpdateState` (legacy) đã tự `INSERT INTO DispatchOrderFCLHistory VALUES (@RefNo,@Step,@UserId,@Feedback,GETDATE())` mỗi lần đổi trạng thái (kể cả từ chối) — comment lấy từ `Migration_FCLStatusLog_20260710.sql`. BE đã đọc lại đủ qua `GetByRefNo`/`GetByRefNoWithTOAsync` (`entity.ListHistory`), nhưng **FE trước giờ KHÔNG hiển thị ở đâu cả** — dữ liệu có ghi nhận đầy đủ (ai/khi nào/lý do gì) nhưng không ai xem lại được.
- **Model** [dispatch-order-fcl.ts](../../src/app/shared/models/fcl/dispatch-order-fcl.ts): +`listHistory?: DispatchOrderFclHistory[]` + interface mới `DispatchOrderFclHistory{id,refNo,step,employeeId,employeeFullName,notes,createdDate}`.
- **Modal mới** `modal-fcl-history` (`src/app/shared/components/transports/modal-fcl-history/`): `.component.ts/.html/.module.ts` theo đúng khuôn skill `deltasoft-modals` (ngDraggable+[handle], lazy-mount, `show(refNo)` gọi lại `dispatchOrderFclService.getDetail(refNo)` rồi lấy `data.listHistory`, sort **mới nhất lên trên**). Bảng 4 cột: Thời gian / Bước / Người thực hiện / Nội dung-Lý do.
- **Gắn nút "Lịch sử"** (cạnh nút "Đính kèm" có sẵn, *ngIf="!flagNew") vào **cả 2** modal legacy: `modal-dispatch-order-fcl` (tạo/sửa) và `modal-perform-fcl` (thực hiện) — thêm field `viewFclHistory`/`ViewChild`/`viewHistory()`/`closeModalFclHistory()` theo đúng pattern `attachFile()` đã có sẵn trong chính 2 file đó. 2 module tương ứng +import `ModalFclHistoryModule`.
- **2026-08-07 — anh tự sửa `SP_DispatchOrderFCL_GetByRefNo`**: câu đọc lịch sử đổi từ `SELECT * FROM DispatchOrderFCLHistory` (không join) thành `SELECT m.*,n.EmployeeFullName FROM DispatchOrderFCLHistory m INNER JOIN V_Users n ON m.UserId=n.Id`. Lý do BE trước đó luôn trả `EmployeeFullName`/`Notes` rỗng: cột thật của bảng là `Step/UserId/Notes` (đọc `INFORMATION_SCHEMA.COLUMNS`, read-only) trong khi Model C# khai `EmployeeId/EmployeeFullName` — sai tên cột nên Dapper map null; JOIN `V_Users` mới bù được tên người thực hiện. Cột `Step` vốn đã là `nvarchar` (text sẵn, không phải mã số).
- Đồng bộ theo SQL mới: BE model [DispatchOrderFCLHistory.cs](../../../NewAPI/API/Models/FCL/DispatchOrderFCLHistory.cs) +`Step` (string); FE interface +`step`; HTML modal +cột "Bước".
- **Anh đang tự bổ sung thêm nhánh trong `SP_DispatchOrderFCL_UpdateState`**: `@CurrentStatus=5` → `@Step=N'Từ chối chốt lệnh'` (tách riêng khỏi nhánh `@TypeRejection` cũ vốn dùng chung cho case này) — SQL anh tự viết/chạy, em không đụng SP legacy này.
- **Verify**: BE `dotnet build -t:Compile` 0 lỗi; FE `npx tsc --noEmit` 6 lỗi pre-existing (không liên quan) + `npx ng build` (default config) 0 lỗi mới, Hash `66c7f59779c7eeaa` (bản build trước khi thêm cột Step — cần build lại lần nữa trước deploy).
- **✅ Đã commit+push cả 2 repo**: web-app-update main `118dff0`/`2ede9db` (FE) · NewAPI master `30c790d` (BE model +Step).
- **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-07): cột "Bước" hiện đúng "Từ chối chốt lệnh", "Người thực hiện" có tên.

## EupfinController — proxy EUP dồn chung 1 controller cho ERP tự dùng + API MỚI phí cầu đường — BE build 0 err, CHƯA COMMIT — 2026-08-06
Nguồn: `NewAPI/Tai lieu ket noi Eupfin_v2.1.7.pdf` mô tả 1 API EUP CHƯA tích hợp — "Truy vấn phí cầu đường theo danh sách xe" (`POST`, header `X-Eupfin-Api-Key`+`Consumer-Id`, body `{carNumbers[],startTime,endTime}` `yyyy-MM-dd HH:mm:ss`, trả `{status,result:[{vehicleNo,startTime,endTime,details[],resultCode,resultMsg}],error}`). Anh chốt: **dồn 1 controller MỚI support tất cả cho ERP** (không phải bề mặt Innvie) — không đụng `GaragesController` (giữ nguyên 100%, Innvie vẫn chạy qua đó).
- **File mới** [EupfinController.cs](../../../NewAPI/API/Controllers/Commons/EupfinController.cs) (`api/Eupfin`, JWT thường qua default policy — như `VietmapApiController`, KHÔNG Api-Key/envelope kiểu Innvie). 5 action dùng chung `AppSettings:EupfinV3*`: `GetRealtimeByCars`/`GetCars`/`GetHistory`/`GetDistance` (port lại logic y hệt 4 action tương ứng trong `GaragesController`, chỉ đổi lớp auth+envelope sang `ResponseValue`) + **`GetTollFee` MỚI**.
- **`GetTollFee`**: path do EUP cấp — anh cho path `/delta/road/cost` → set `AppSettings:EupfinTollFeePath="/road/cost"` (base đã có sẵn `/delta` nên nối ra đúng `.../gateway2/delta/road/cost`). Test thật (script scratchpad, không in key) xe `29E10562` → `resultCode:1 INVALID_DEVICE`.
- **★ Phát hiện qua test thật**: EUP đăng ký biển kèm hậu tố thiết bị (`/cars` trả `"29E10562(CAM)"`, KHÔNG phải biển trơn) — gọi bằng biển trơn từ DB ERP sẽ trật. **Đã thêm cơ chế dò-lại-khi-lỗi**: gọi lần 1 bằng biển trơn → dòng nào `resultCode≠0` mới gọi `/cars` (chỉ khi có lỗi, không gọi mỗi request) → so khớp kiểu "1 bên là tiền tố bên kia" sau khi bỏ ký tự đặc biệt (`PlateMatches`, vì hậu tố `(CAM)` có chữ nên không thể so `==`) → khớp ĐÚNG 1 ứng viên thì retry batch 1 lần, **giữ nguyên `vehicleNo` gốc** trong kết quả trả về (chỉ thay data phí) để caller đối chiếu ngược input; khớp NHIỀU ứng viên (vd vừa có bản trơn vừa `(CAM)`) → KHÔNG đoán, trả nguyên lỗi kèm danh sách ứng viên trong `resultMsg`.
- **Verify test thật xe `29E10562(CAM)`, 2026-08-04→hiện tại**: `resultCode:0 SUCCESS`, 9 lượt qua "Trạm Thu Phí Số 2 QL5" (mã trạm 484/485), `cost=0` cả 9 (không phải lỗi kỹ thuật — có thể xe diện miễn phí trạm này). Mô phỏng lại đúng thuật toán dò-lại bằng script riêng — xác nhận biển trơn `29E10562` tự dò ra đúng 1 ứng viên `29E10562(CAM)` → retry → khớp y hệt kết quả gọi trực tiếp.
- **Model mới** trong `EupClass.cs`: `EupfinVehicleRequest`, `EupTollFeeRequest`, `apiEupTollFeeResult` (tái dùng `Result`/`Detail` — 2 class này đã có sẵn từ trước, khớp gần đúng cấu trúc tài liệu nhưng **chưa từng được dùng ở đâu**, coi như dọn nợ code chết cũ).
- Config: thêm `AppSettings:EupfinTollFeePath` vào `appsettings.Development.json` + `appsettings.Production.json` (2 file gitignore, không lộ secret).
- ⚠ **Sự cố nhỏ trong phiên**: lỡ echo API Key + Consumer-Id ra output 1 lần lúc debug (lệnh `grep`) — đã báo anh ngay, đề nghị anh cân nhắc đổi key nếu thấy cần.
- **✅ ĐÃ COMMIT+PUSH+PUBLISH, TEST OK** (anh xác nhận 2026-08-07).

## FCL v2 — API lưu TỪNG dòng chi phí (per-line CreateFeeV2/UpdateFeeV2/DeleteFeeV2) — SQL soạn xong (anh DUYỆT) + BE build 0 err ✅ ĐÃ COMMIT+PUSH, CHỜ chạy SQL + redeploy API — 2026-08-05
> **Commit/push 2026-08-05**: BE NewAPI **master `4f683cc`** (controller multipart + repo/interface/model + FromForm + SQL migration + prompt) · docs web-app-update **main `7d56bf2`** (skill `fcl-mobile-api` §4.8 + done/todo). draft-web KHÔNG đụng.
...
- **★ CreateFeeV2/UpdateFeeV2 = MULTIPART `[FromForm]`** (anh chốt — PDA POST 1 LẦN kèm cả data lẫn ảnh): form `Item`(JSON dòng phí, **KHÔNG** có PathFile) + `File`(IFormFile tùy chọn) + `TokenKey`. BE tự: có File → `SaveFeeFileAsync` (lưu local + S3) → gán `PathFile` vào dòng phí → INSERT/UPDATE; không File → CreateV2 PathFile NULL / UpdateV2 giữ ảnh cũ. Trả `data.Id` + `data.PathFile`. Model mới `FromBodies/FCL/FromFormFclFeeV2.cs`.
- **★ SỬA 2026-08-06 (anh chốt, trước redeploy nên sửa thẳng, KHÔNG tạo v3)**: `SaveFeeFileAsync` ban đầu tạo record `DispatchOrderAttachFiles` (IsPod=false) — tức là ảnh hóa đơn per-line lại nhảy vào **"Ảnh hiện trường"** của lệnh (giống POD). Anh yêu cầu: phải vào đúng **"Đính kèm hồ sơ"** — hệ thống `AttachFiles` GENERIC (`FrmName='FCL', FunctionName='FCL', RefNo=<refNo lệnh>`) mà nút "Đính kèm hồ sơ" trên modal tạo lệnh (`attachFile()` trong `modal-dispatch-order-fcl-v2.component.ts:1576`) đang dùng — 2 bảng khác nhau, KHÔNG liên quan `DispatchOrderAttachFiles`/POD. Đã sửa: `DispatchOrderFCLController` đổi DI `IDispatchOrderAttachFiles`→`IAttachFiles`; `SaveFeeFileAsync` insert `AttachFiles{FrmName="FCL",FunctionName="FCL",RefNo,UserId=User.GetUserId(),BranchId=User.GetBranchId()??0,Size,PathFile}` qua `SP_AttachFiles_Create` (SP có sẵn, KHÔNG cần SQL mới). File vật lý (đĩa+S3) không đổi.
- **★ SỬA TIẾP 2026-08-06 (cùng phiên)**: bảng `AttachFiles` gộp chung theo `RefNo` lệnh, không có cột "dòng phí" ⇒ anh hỏi làm sao phân biệt ảnh nào ứng với dòng phí nào khi liệt kê "Đính kèm hồ sơ". Giải pháp KHÔNG ALTER schema — mượn 2 cột có sẵn nhưng chưa dùng cho FCL: **`AttachFiles.JobId` = Id dòng phí** (`DispatchOrderFCLFee.Id`, dạng chuỗi) để lọc/liên kết ngược, **`AttachFiles.Title` = `Contents`** (nội dung dòng phí) để hiện tên gợi nhớ khi liệt kê. `SaveFeeFileAsync` thêm 2 tham số `feeLineId`/`title`. Hệ quả đổi thứ tự **CreateFeeV2**: lúc lưu ảnh vẫn CHƯA có Id (chưa insert) ⇒ đảo thành **INSERT dòng phí trước (PathFile=NULL) lấy `newId`** → có File thì lưu ảnh (gắn `JobId=newId`) → **UPDATE lại `PathFile`** vào đúng dòng (2 lượt SQL thay vì 1, chỉ khi có ảnh). **UpdateFeeV2** không đổi thứ tự (đã có `item.Id` sẵn từ đầu). `dotnet build -t:Compile` → 0 Error (2 lần).
- **BE (build 0 err, ✅ ĐÃ COMMIT+PUSH master `4f683cc`, + 2 lượt sửa attach 2026-08-06 CHƯA commit)**: `IDispatchOrderFclFee` +`CreateV2/UpdateV2/DeleteV2`; `DispatchOrderFCLFeeRepository` +3 method (CreateV2 dùng `ExecuteScalarAsync<int>` lấy Id; helper `FeeV2Params`); `DispatchOrderFCLController` +`CreateFeeV2`/`UpdateFeeV2` (multipart) +`DeleteFeeV2` (FromBody) + DI `IConfiguration`/`IS3StorageService`/`IAttachFiles` + helper `SaveFeeFileAsync(file,refNo,feeLineId,title)`/`RandomString`; `DispatchOrderFCLRepository.DriverUpdate` +truyền `@SkipFeeMerge`; model `DispatchOrderFCL` +`bool? SkipFeeMerge`.
- ⚠ **Gap còn để yên (không trong phạm vi hôm nay)**: `DeleteFeeV2` chỉ xóa dòng phí, KHÔNG dọn record `AttachFiles` tương ứng (mồ côi) — giống hệt hạn chế cũ của `DispatchOrderAttachFiles`/POD trước giờ, không phải lỗi mới.
- **✅ ĐÃ DEPLOY + TEST OK** (anh xác nhận 2026-08-07): commit fix attach, chạy SQL, publish API, mobile đổi handler xong. Web không đổi.
...
- **Tài liệu cho Innvie (2026-07-28)**: `NewAPI/Garage_API_Documentation_APImoi_2026-07-27.docx` (+ `.md` nguồn) — bổ sung `Garage_API_Documentation.docx` gốc, tự chứa đủ nhóm API phương tiện/GPS: **mục 9** VehicleGetDistance (số km tích lũy, giữ hợp đồng cũ) · **mục 10** GetRealtimeByCars (+3 field mới v3 driverName/instantFuel/totalMile) · **mục 11** GetCars (MỚI) · **mục 12** GetHistory (MỚI) + bảng mã carStatus + bảng tổng hợp. Số liệu ví dụ từ test thật, KHÔNG lộ credential EUP. Dựng bằng Word COM (HTML→docx, máy không có pandoc). File docx untracked ở NewAPI, chưa commit.
...
> **2 ngoại lệ có chủ đích:** ⏸ `Migration_AccountingEntry_P50_20260710.sql` (bút toán P5.0 — TẠM DỪNG, chưa chạy) · ⬜ grant `FCL_ACCEPT` cho role `DV`/`DV-M` (anh tự bổ sung; chưa có ⇒ điều vận chưa thấy nút Duyệt B1).
> **Verify read-only cùng ngày**: `_Update` chốt dầu đúng `B+C−A` & query kiểm lệch **0 dòng** · `SP_DriverFuelApproval_Cancel` · `ChangeStatus` (+`@EmployeeId`) · `GetAll` (+`@IsLegacy/@IsSubcontractors/@SupplierId`) · `CreateWithTO` (+`RecomputeTotals`, status khởi tạo=1) · `SP_GetAllLocations` (+`@BranchId`) · `Ports.BranchId` (82 chung/12 CN5/1 CN6) · `draft.SP_DraftEntries_Insert` (+`ShippingTask`) · 0 lệnh FCL v2 kẹt `Status=0`.

## Draft Site — Nháp ShippingTask (loại nháp thứ 5) — SQL ĐÃ CHẠY + config áp xong + draft-web FE XONG, chờ build/deploy + P4/P5 ERP — 2026-07-22
"Chuyến GNVC nháp": AI/người nhập tạo nháp chuyến trên site nháp → người duyệt promote thành ShippingTask thật trong ERP. Clone khuôn 4 loại nháp cũ; hạ tầng draft generic dùng lại ~100%.
- **P0 SQL ✅ ĐÃ CHẠY** (verify 2026-07-24: `draft.SP_DraftEntries_Insert` đã cho `@DraftType='ShippingTask'`) [Migration_DraftSite_ShippingTask_20260720.sql](../../../NewAPI/Migration_DraftSite_ShippingTask_20260720.sql): DROP+CREATE `draft.SP_DraftEntries_Insert` — thân VERBATIM (đã đối chiếu OBJECT_DEFINITION live = khớp Phase0, KHÔNG bị sửa tay), delta duy nhất `@DraftType NOT IN (...,'ShippingTask')`. Verify DB: bảng `DraftEntries` KHÔNG có CHECK constraint, cột VARCHAR(20) đủ; `draft_app` có EXECUTE cấp SCHEMA::draft ⇒ DROP+CREATE không mất quyền. 5 SP còn lại + `dbo.SP_DraftEntries_GetForErp_GetPaging` generic theo @DraftType → không đụng. ⚠ Chạy TRƯỚC khi restart DraftAPI với appsettings mới.
- **P1 DraftAPI (config, chờ áp)**: thêm `"ShippingTask": { "Create": "WORKFLOW_CREATE", "Update": "WORKFLOW_UPDATE", "Delete": "WORKFLOW_DELETE" }` vào `Draft:Permissions` (appsettings.json). ShippingTask KHÔNG có FunctionCode riêng — Create/Update/Delete ERP gate bằng `FunctionCode.WORKFLOW`. Config-only, chỉ restart.
- **P2 ERP ReadAllowlist (config, chờ áp)**: thêm `"/api/ports/getall"` + `"/api/customerlocations/getall"` vào `Draft:ReadAllowlist` (NewAPI/API/appsettings.json). 2 endpoint này KHÔNG có ClaimRequirement ⇒ default-deny chặn nếu không allowlist. 4 lookup còn lại (customer/handlinggroup/othercategories/employee) token nháp đã đọc được. Chỉ restart ERP API. (Xe/lái xe KHÔNG cần — là việc OpMan lập sau, modal CS không đụng.)
- **P3 draft-web FE — XONG (ng build 0 lỗi)**: đổi module `shipping-task/` từ list readonly (mirror chuyến ERP cho DraftTaskDocs) → **tạo nháp**. Port TRỌN modal CS thật `modal-shipping-task-cs` + giữ hành vi (chọn KH→reset lô + load `customerLocations`+`promotedDraftShipments`; chọn lô→fill jobId/weight/type/booking + set `isEport` khi shipmentType=1174; `isEport` đổi nguồn Nơi nhận/giao giữa Ports↔CustomerLocations + đổi field T/g trả cont↔Cut-off; 4 mốc ngày `dd/MM/yyyy HH:mm:ss`). Files: `core/models.ts` (+`ShippingTaskDraftPayload`/`PortLookup`/`CustomerLocationLookup`, DraftType union +'ShippingTask'), `core/lookup.service.ts` (+`ports()`/`customerLocations(customerId)` — body khớp ERP: ports `item:{branchId:0}`, locations `bValue:true,item:{customerId,id}`), **mới** `shipping-task-form.ts/html`, viết lại `shipping-task-list.ts/html` (nguồn `getPaging({draftType:'ShippingTask'})`, nút Sửa/Xóa chỉ Draft), `layout.html` (nhãn "CV GNVC (nháp)"). Route giữ path `shipping-task` (cùng class name). Chọn lô = `promotedDraftShipments` (khớp sibling Payment/Debit/Workflow) + filter `[1174,1175,1177,46]`. Header "Nhân viên (lập hộ)" như 4 form kia (scope view ERP). ⚠ `shipping-task-docs-modal.ts` GIỮ trên đĩa (orphan, docs làm sau) — build tree-shake, không lỗi.
- **Sửa sau khi test thực tế (2026-07-22)**:
  - **Nguồn chọn Lô hàng ĐỔI** `promotedDraftShipments` → **lô THẬT của ERP**. ShippingTask tham chiếu lô có sẵn (không phải lô nháp). Khớp đúng ERP: modal `modal-shipment-view-search.loadShipment()` → `getAllByCustomer` → POST `/api/shipment/getall` → SP `SP_Shipment_GetAll`. Thêm `LookupService.shipmentsByCustomer(customerId, branchId)` + `"/api/shipment/getall"` vào `Draft:ReadAllowlist` (route này `[ClaimRequirement]` bị comment ⇒ default-deny chặn). Bỏ lọc trước theo loại hình — hiện đủ list, validate khi chọn (giống `saveSuccessJobId` ERP).
  - **Thêm ô "Chi nhánh" trên form nháp** (bắt buộc). Nguyên nhân: `SP_Shipment_GetAll` có `INNER JOIN ShipmentBranchs b ON b.BranchId=@BranchId` ⇒ **sai/thiếu chi nhánh là 0 dòng**; token nháp login `branchId` có thể =0. SP KHÔNG dùng `@UserId` nên không phải lỗi quyền. Người nhập/AI chọn chi nhánh y như lúc đăng nhập ERP; đổi chi nhánh → reset lô + load lại. `models.ts`: `branchId?: number | null`.
  - **Modal cuộn được**: `modal-xl + centered` → **`modal-fullscreen modal-dialog-scrollable`**, VÀ thẻ `<form>` bọc giữa `.modal-content` và header/body/footer phải là flex-column (`d-flex flex-column; min-height:0; flex:1 1 auto; overflow:hidden`) — form chen vào **cắt đứt chuỗi flex** khiến `modal-dialog-scrollable` mất tác dụng, dòng cuối bị lấp. (debit/payment form không có `<form>` bọc nên vốn chạy tốt.)
  - **Bỏ checkbox "Chuyển việc cho Điều vận"** khỏi nháp — nháp luôn `status = 0`; việc đổi status do ERP đặt lúc xác nhận promote.
- **Fix kèm — Debit promote `Step`**: `DebitNoteController.AddFromDraft` giờ ép `obj.Item.Step = 0` (giống ERP tạo mới, FE luôn khởi tạo `step: 0`). Trước đó lấy nguyên payload ⇒ AI bot quên field là ra `Step = NULL`; SP `SP_DebitNotes_Create` có `@Step int = NULL` và dòng `--SET @Step=0` **đã comment** nên không cứu. `NULL ≠ 0`: điều kiện `value.Step < 1` (cho phép sửa/duyệt) với NULL ra UNKNOWN ⇒ debit promote bị kẹt.
- **✅ P4 ERP — View + Sửa + Promote nháp (màn CS) XONG (2026-07-25, BE `dotnet build -t:Compile` 0 err + FE `ng build` 0 err, chờ deploy + test E2E)**. Anh chốt 3 điểm: host = **màn CS `shipping-task-cs`** (KHÔNG opman — nháp do CS tạo, list này dùng đúng `modal-shipping-task-cs`); promote = **CS sửa được nháp + tự tick "Chuyển việc cho Điều vận"** (status 0/1); `MarkPromoted` → **PromotedShipmentId = Id CHUYẾN vừa tạo** (giống khuôn Shipment; sửa 2026-07-25 BE `ec27708` — ban đầu ghi ShipmentId lô ⇒ draft hiển thị sai, không trỏ tới task đã tạo), **PromotedRefNo = ReferCode chuyến** (fallback JobId lô).
  - **BE** [ShippingTaskController.AddFromDraft](../../../NewAPI/API/Controllers/Transports/ShippingTaskController.cs) (+inject `IDraft`): clone `ShipmentController.AddFromDraft`, `[ClaimRequirement(WORKFLOW, CREATE)]`, guard idempotent `GetForPromote`, `ShipmentId>=1`, `BranchId` giữ từ payload (fallback token), `Status` từ payload (FE set theo checkbox) → `CreateAsync` → nếu `Status==1` bắn `SendNotificationToFirebase` (giống Create gốc) → `MarkPromoted(draftId, obj.KeyWord=jobId, obj.Item.ShipmentId, reviewer)`. **Ngày KHÔNG reformat** (`CreateAsync` parse `dd/MM/yyyy HH:mm:ss` = đúng định dạng payload nháp). KHÔNG SQL mới, KHÔNG đụng Create/SP/mobile.
  - **FE service** [shipping-task.service](../../src/app/shared/services/transports/shipping-task.service.ts): +`addFromDraft(entity, draftId, jobId)` → POST `/api/shippingtask/addFromDraft` (`item/id/keyWord`).
  - **FE modal** [modal-shipping-task-cs](../../src/app/shared/components/transports/modal-shipping-task-cs/modal-shipping-task-cs.component.ts): +`_isDraftView/_draftId` + `@Output() ApproveDraft` + `viewDraft(payload, draftId)` (parse payload→entity, **`flagXem=false` cho sửa**, `_ensureDraftCustomer` bơm KH ngoài phạm vi, `formatAndSetDateTime` cho picker, `entity.id=undefined`) + `approveDraft()` (confirm → `status = checked?1:0` → `addFromDraft` → emit) + reset `_isDraftView=false` ở `add()/edit()` + guard `if(_isDraftView) return` đầu `saveChange/saveAndNew`. HTML `[class.draft-view]` nền vàng + badge **NHÁP #id** + nút **"Xác nhận chuyển sang ERP"** (ẩn Lưu/Lưu&Nhập tiếp). CSS `.draft-view`.
  - **FE list** [shipping-task-cs.component](../../src/app/main/transports/shipping-task-cs/shipping-task-cs.component.ts): `loadData` = `forkJoin({erp: getAllByCs, draft: getPagingForErp({draftType:'ShippingTask'})})` (draft `catchError→[]`); `mapDraftToRow` (parse ngày `pickupTime/deliveryTime`→Date để `DatePipe` KHÔNG vỡ `*ngFor`); trộn `[...draftRows, ...erp]`; `clickRow` guard `_isDraft`; badge "NHÁP #" + nút "Xem/Duyệt" → `showDraft`→`viewDraft`; `onConfirmPromote`→đóng modal+`loadData`. HTML nền vàng + guard nút Copy/Hủy việc/Đính kèm cho dòng nháp. Model +`_isDraft/_draftId/_draftPayload`.
  - **★ Fix ngày (2026-07-25, FE main 803a8c0)**: `viewDraft` gọi `formatAndSetDateTime` (parse **EN `MM/DD`**) trên payload nháp dạng **VN `DD/MM/YYYY HH:mm:ss`** ⇒ ngày >12 thành "Invalid date", ngày ≤12 bị **đảo ngày↔tháng** ⇒ BE `ParseExact` VN hỏng → **mốc thời gian invalid khi promote**. Thay bằng `_setupDraftDate()`: parse khoan dung (ưu tiên VN + ISO), giữ chuỗi `DD/MM/YYYY HH:mm:ss` (đúng `CreateAsync` ParseExact) + dựng option picker. Áp cho cả 7 field (est start/finish, pickup, delivery, cutoff, demurage, detention — verify `SP_ShippingTask_Create` ParseExact cả 7).
  - **CÒN**: ⬜ tắt API → build/publish (endpoint mới) + `ng build` deploy FE → test E2E (tạo nháp draft-web → màn CS thấy dòng vàng → mở sửa field → tick "giao Điều vận" → Xác nhận → chuyến thật status=1 + OpMan nhận noti; nháp biến mất; draft-web hiện JobId/ShipmentId lô; bấm 2 lần không tạo trùng). ⬜ **P5 duyệt hàng loạt** (checkbox chọn nhiều + lặp `AddFromDraft`) — chưa làm, để sau khi test P4 ổn.
- ~~CÒN LÀM cũ~~: P0 SQL đã chạy; draft-web/DraftAPI đã deploy (mốc 2026-07-24).

## Cấp dầu theo lệnh — Hủy phiếu ĐÃ XUẤT + nhả lệnh về GetForSummary — SQL đã chạy, BE+FE build 0 lỗi, ĐÃ COMMIT+PUSH, chờ deploy — 2026-07-16
Phiếu cấp dầu theo lệnh (`DriverFuelApproval` Type=2) đã xuất thì Hủy được, lệnh trong phiếu hủy hiện lại khi tạo phiếu mới.
- **Gốc rễ** (agent map backend): `SP_DispatchOrder_GetForSummary` loại lệnh bằng **2 lớp AND** — (a) cờ `IsSummarized`/`IsGeneratorSummarized` trên bảng lệnh, (b) `RefNo` chưa nằm trong phiếu còn hiệu lực (`n.Deleted=0 AND n.Status>-1`). Bước Xuất phiếu (`SP_DriverFuelApproval_Update @Status=1`, [Migration_FCL_GeneratorSummarized_20260629.sql:220-241]) set cờ (a)=1. Hủy cũ (`huyPhieu`→-1 qua Update, `cancelExpired`→-2) **không reset cờ** → lớp (b) đã nhả nhưng lớp (a) vẫn ẩn lệnh vĩnh viễn. Đây là lý do lệnh phiếu đã hủy không hiện lại.
- **Anh chốt (2026-07-16)**: hủy thường → **Status=-1** (giữ -2 riêng cho `cancelExpired` IGAS quá hạn) · áp cả IGAS lẫn NCC thường · **chặn khi đã đổ** (`QuantityIgas>0`) · bắt lý do + quyền `DRIVERFUELAPPROVAL_ACCEPT`.
- **SQL** (✅ anh đã chạy) [Migration_DriverFuelApproval_Cancel_20260716.sql](../../../NewAPI/Migration_DriverFuelApproval_Cancel_20260716.sql): SP MỚI `SP_DriverFuelApproval_Cancel` (validate Type=2/đã xuất/chưa chốt/chưa đổ → reset 4 nhóm cờ gương đúng khối set, **có guard "không nhả lệnh đang bị phiếu khác giữ"** `RefNo NOT IN phiếu khác Status>-1` → Status=-1 + append lý do vào Note). Không đụng SP cũ (`_Update`/`_Create2`/`_Approved`/`_CancelExpired`/`_Delete`/`GetForSummary`).
- **BE**: `POST /api/DriverFuelApproval/Cancel` (interface `Cancel` + repo + controller), quyền ACCEPT trong `[ClaimRequirement]`, bắt `System.Data.SqlClient.SqlException`→400 (hiện đúng câu RAISERROR của SP). Build 0 lỗi.
- **FE** [modal-fuel-summary](../../src/app/shared/components/transports/modal-fuel-summary/modal-fuel-summary.component.ts): `huyPhieu()` gọi `cancel()` (bắt lý do, success set -1 + `SaveSuccess.emit` reload); service +`cancel(id,reason)`. Bỏ đường update→-1 cũ (không reset cờ + bug success gán =1). **2 nút loại trừ nhau theo hạn đổ** — helper `isIgasExpired()` (`isLocal && igasCode && now > validTime`; validTime = "Hạn đổ"): NCC thường (isLocal=false, không IGAS) & IGAS còn hạn → **"Hủy phiếu"** (-1); IGAS **quá hạn** → ẩn nút đó, hiện **"Hủy IGAS hết hạn"** (-2). Cả 2 nút gate `apprved_permission && status==1 && !quantityIgas`.
- **Verify**: FE tsc + `ng build` 0 lỗi (Hash 14799b48); BE `dotnet build -t:Compile` 0 Error. **Commit+push**: FE `main` 81f6f2b · BE `master` 740bdf4.
- ⬜ **CÒN**: tắt API → build/publish (endpoint mới) + deploy FE → test E2E (xuất → Hủy nhập lý do → tạo phiếu mới cùng xe: lệnh hiện lại; phiếu đã đổ/đã chốt: SP chặn).

## Chốt dầu phương tiện (F039) — Sửa phiếu nháp: hiện lại nút Tải/xóa dòng + khóa đổi xe + "Tải dữ liệu" chạy MERGE — FE, ĐÃ COMMIT+PUSH, chờ deploy — 2026-07-16
Anh báo: mở phiếu nháp ra Sửa thì mất nút "Tải dữ liệu", và nếu tải lại thì mất các dòng đã có (SP `GetCandidates` loại RefNo đã nằm trong phiếu → tải về chỉ có dòng mới, code cũ ghi đè → mất dòng cũ khi Lưu).
- [modal-vehicle-fuel-closing.component.html](../../src/app/shared/components/transports/modal-vehicle-fuel-closing/modal-vehicle-fuel-closing.component.html): nút "Tải dữ liệu" + cột xóa dòng (th+td) đổi gate `flagNew && !flagXem` → `!flagXem && entity?.status !== 1` (hiện cả khi Sửa nháp, ẩn khi Xem/Đã duyệt); dropdown **Xe** đổi disable `flagXem || (!flagNew && status===1)` → `flagXem || !flagNew` (khóa đổi xe khi Sửa — đổi xe = tạo phiếu mới); nút đổi nhãn động "Tải dữ liệu chưa tổng kết" (mới) / **"Bổ sung dữ liệu mới"** (sửa).
- [modal-vehicle-fuel-closing.component.ts](../../src/app/shared/components/transports/modal-vehicle-fuel-closing/modal-vehicle-fuel-closing.component.ts): `loadCandidates()` khi `flagNew` = thay toàn bộ (như cũ); khi **Sửa = MERGE** — giữ nguyên `entity.detaileds`, chỉ append dòng mới (dedupe `source+refNo`), toast "Đã bổ sung N dòng…". Tránh mất dòng cũ của phiếu.
- **Chốt phương án** (anh 2026-07-16): giải quyết ở FE (merge + xóa tay dòng sai) là đủ; **SP `GetCandidatesForUpdate` — DEFERRED**, chỉ dựng nếu vận hành phát hiện dòng nguồn vô hiệu (lệnh nguồn bị xóa/đảo status) lọt phiếu mà không ai thấy (case hiếm, nguồn đều status 6 đã khóa).
- **Verify**: tsc + `ng build` 0 lỗi. Commit+push FE `main` 81f6f2b (chung commit với Hủy phiếu). ⬜ CÒN: deploy FE.
...
- **⚠ VÁ `SP_DispatchOrderFCL_ChangeStatus` ĐANG THIẾU TRÊN DB**: kiểm DB thấy bảng `Tbl_DispatchOrderFCLStatusLog` có nhưng SP `ChangeStatus` **không có** → `Migration_FCLStatusLog_20260710.sql` chạy DỞ. Nếu deploy mà không vá thì mọi thao tác workflow lỗi "không tìm thấy SP". File trên **dựng lại SP verbatim** → vá luôn. (⚠ Nên rà các object khác của đợt 07-10 vd `SP_DispatchOrderFCLStatusLog_GetByRefNo` cho timeline.)
- **Ảnh chứng từ theo TỪNG dòng chi phí** (`DispatchOrderFCLFee.PathFile`): app chụp/upload → `pathFile` gán vào dòng, gửi trong `listFee`. SQL [Migration_FCL_FeePathFile_20260712.sql](../../../NewAPI/Migration_FCL_FeePathFile_20260712.sql): +cột `PathFile` + `TypeDispatchOrderFCLFeeV2` **18 cột** + SP `DriverUpdate` MERGE thêm PathFile (surgical, không đụng Create/UpdateWithTO). FE: cột **"Ảnh"** — nút Tải/chụp (`accept=image/*,.pdf`, 1 nút lo cả 2) / 📎 xem + xóa liên kết; upload dùng lại `DispatchOrderAttachfiles/create` (isPod=false) lấy `data[0].pathFile`. **Bắt buộc đính ảnh khi `invoiceType=2`** (validate chặn Lưu, cùng 6 field HĐ). Ảnh vẫn vào list "Ảnh hiện trường"; POD giữ nguyên.
- **Fix màu list chốt dầu phương tiện** [vehicle-fuel-closing.component.html](../../src/app/main/transports/vehicle-fuel-closing/vehicle-fuel-closing.component.html): cột Net(lít) tô **ngược** (đỏ khi >0) → sửa **>0 xanh (nhận) / <0 đỏ (thu)**; cột Net Amount trước không màu → thêm màu + in đậm, khớp modal `modal-vehicle-fuel-closing`.
- **Verify**: BE build succeeded (scratch), FE tsc 0 lỗi. SQL anh đã chạy hết; BE+FE anh báo đã deploy. Đã push (FE `main` 3c6c91a, BE `master` 869a0aa).
...
## Danh mục Cảng bãi — phân loại theo **CHI NHÁNH** (`Ports.BranchId`) — SQL ✅ ĐÃ CHẠY, BE 0 err + FE 0 err — 2026-07-10
> Verify read-only 2026-07-24: `Ports.BranchId` đã có dữ liệu — **82 cảng NULL (dùng chung) + 12 cảng CN5 + 1 cảng CN6**; `SP_GetAllLocations` có đủ `@ListCust` + `@BranchId`.
