...
- **CÒN**: `ng build` production + deploy + test thật: xuất phiếu → không đổ, nhập S/L thực tế=0 → bấm Chốt dữ liệu → chốt thành công (trước đây bị chặn im lặng).
...
- **CÒN**: `ng build` production + deploy + test thật: Lưu lệnh mới không cần chọn chặng cuối → Duyệt B1 bị chặn nếu chưa chọn chặng cuối (tooltip) → sau Duyệt B1 mọi ô dầu/phí/tóm tắt/ghi chú/route đều khóa dù mở ở chế độ sửa.
